...
std::string ValueFromAmountString(const CAmount& amount, const int8_t units)
{
    ...
    static constexpr int64_t DIVISORS[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000}; 
    remainder = remainder / DIVISORS[8-units]; // avoids FPU ops
...
std::string ScriptToAsmStr(const CScript& script, const bool fAttemptSighashDecode)
{
    ...
    while (pc < script.end()) {
        ...
        if (opcode == OP_SOTER_ASSET) {
            // Once we hit an OP_SOTER_ASSET, we know that all the next data should be considered as hex
