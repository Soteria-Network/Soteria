...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::ConsensusParams& params) {
    ...
    int64_t nPastBlocks = 180; // even though we don't use DGW at all but if you need it in the future let it use same avg window
...
unsigned int GetNextWorkRequiredLWMA(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::ConsensusParams& params, const POW_TYPE powType)
{
    ...
    if (!pindexLast || pindexLast->nHeight >= params.diffRetargetStartHeight1 && pindexLast->nHeight < params.diffRetargetEndHeight1)
    ...
    const POW_TYPE powType)
{
 ...
 // const size_t TYPE_COUNT = sizeof(POW_TYPE_NAMES) / sizeof(POW_TYPE_NAMES[0]);
    ...
    int64_t now = GetTime();
    bool logThis = verbose && (now - lastLogTime[static_cast<size_t>(powType)] > 10); // def=30
    ...
    const POW_TYPE powType)
{
 ...
 // const size_t TYPE_COUNT = sizeof(POW_TYPE_NAMES) / sizeof(POW_TYPE_NAMES[0]);
    ...
    bool verbose = LogAcceptCategory(BCLog::SOTERC_SWITCH);
    int64_t now = GetTime();
    bool logThis = verbose && (now - lastLogTime[static_cast<size_t>(powType)] > 15); // def=30
    if (logThis) lastLogTime[static_cast<size_t>(powType)] = now;
    ...
    const POW_TYPE powType)
{
    ...
    const int64_t spacingPerAlgo = params.nPowTargetSpacing * 2;
    const int64_t T = spacingPerAlgo;
    const int64_t N = 180; // params.lwmaAveragingWindow
    const __int128 k = (__int128(N) * (N + 1) / 2) * T;             // weighted sum timespan
    const int64_t height = pindexLast->nHeight + 1; 
...
unsigned int GetNextWorkRequiredLWMA4(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::ConsensusParams& params, const POW_TYPE powType) {
    ...
    const int64_t N = 180;                                                          // Window size
...
unsigned int GetNextWorkRequiredLWMA10(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::ConsensusParams& params, const POW_TYPE powType) {
    ...
    static std::map<POW_TYPE, int64_t> lastLogTime;
    const int64_t now = GetTime();
    const bool logThisCall = verbose && (now - lastLogTime[powType] > 15);
    if (logThisCall) lastLogTime[powType] = now;
...
// use soterc as a min default with 4x loose value & 8x harder
...
{    
    ...
    bool verbose = LogAcceptCategory(BCLog::SOTERC_SWITCH);
    int64_t now = GetTime();
    bool logThis = verbose && (now - lastLogTime[static_cast<size_t>(powType)] > 15);
    if (logThis) lastLogTime[static_cast<size_t>(powType)] = now;
    ...
    if (height < N + 1) {
        if (logThis) LogPrintf("* Short chain [%d < %d], using powLimit\n", height, N+1);
        return powLimit.GetCompact();
    }
    ...
    if (blocks.size() < (size_t)(N + 1)) {
        if (logThis) {
            LogPrintf("* Found only %zu/%d same-type blocks, using powLimit\n",
                      blocks.size(), N+1);
        }
        return powLimit.GetCompact();
    }
    ...
    arith_uint256 minTarget = powLimit >> 3; // allow up to 4x harder than powLimit (tunable)
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* nextTarget below minTarget, raising\n");
        nextTarget = minTarget;
    }
    if (nextTarget > powLimit) {
        if (logThis) LogPrintf("* nextTarget above powLimit, clamping\n");
        nextTarget = powLimit;
    }
    ...
    if (useEWMA) {
        ...
        if (finalTarget > powLimit) finalTarget = powLimit;
    }
...
// LWMA15 soterg 180-180 blocks weight 25%
...
// LWMA17 soterc 30-180 blocks 25% weight
    ...
    const POW_TYPE powType)
{
 ...
 // const arith_uint256 powTypeLimits[powType] = UintToArith256(params.powTypeLimits[POW_TYPE_SOTERG]);
   ...
   arith_uint256 nextTarget   = (numerator + halfK) / K;
    ...
    // Clamp to [minTarget, powLimit]
    arith_uint256 minTarget = powLimit >> 3; // 2
  // no harder than 2× the limit, def =2, 1 = 2× harder, 2 = 4× harder, 3 = 8× harder, 4 = 16× harder
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* Raising target to lower clamp\n");
        nextTarget = minTarget;
...
// LWMA18 soterc 30-180 blocks 50% weight
...
unsigned int GetNextWorkRequiredLWMA21(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
...
// soterg 180-300 blocks 50% weight
unsigned int GetNextWorkRequiredLWMA22(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
...
unsigned int GetNextWorkRequiredLWMA23(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
...
unsigned int GetNextWorkRequiredLWMA24(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
...
// use soterg as a min default with 4x loose value & 8x harder than powLimit
unsigned int GetNextWorkRequiredLWMA25(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
{    
    ...
    // will allow reasonable hardness change and loosened while testing
    arith_uint256 minTarget = powTypeLimit >> 3; // allow up to 8x harder than powLimit (tunable)
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* nextTarget below minTarget, raising\n");
        nextTarget = minTarget;
    }
...
// use soterc as a min default with 6x loose value & 8x harder than powLimit
...
{    
    ...
    // will allow reasonable hardness change and loosened while testing
    arith_uint256 minTarget = powLimit >> 3; // allow up to 8x harder than powLimit (tunable)
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* nextTarget below minTarget, raising\n");
        nextTarget = minTarget;
    }
...
// use soterc as a min default with 4x loose value & 4x harder
...
{    
    ...
    // will allow reasonable hardness change and loosened while testing
    arith_uint256 minTarget = powLimit >> 2; // allow up to 4x harder than powLimit (tunable)
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* nextTarget below minTarget, raising\n");
        nextTarget = minTarget;
    }
...
// use 000000fff as default diff for both algos
...
// use 000000fff as default diff for both algos 4t
...
unsigned int GetNextWorkRequiredLWMA37(
    const CBlockIndex* pindexLast,
    const CBlockHeader* pblock,
    const Consensus::ConsensusParams& params,
    const POW_TYPE powType)
...
// use soterg as a min default with 4x loose value & 4x harder than powLimit
...
{    
    ...
    // will allow reasonable hardness change and loosened while testing
    arith_uint256 minTarget = powTypeLimit >> 2; // allow up to 4x harder than powLimit (tunable)
    if (nextTarget < minTarget) {
        if (logThis) LogPrintf("* nextTarget below minTarget, raising\n");
        nextTarget = minTarget;
    }
...
// use soterc as a min default with 6x loose value & 4x harder than powLimit
