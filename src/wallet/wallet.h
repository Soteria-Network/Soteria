...
static constexpr bool DEFAULT_SPEND_ZEROCONF_CHANGE = false;
//! Default for -walletrejectlongchains, Reject chains of unconfirmed change to reduce risks of stuck funds and accidental RBF loops. def=false
...
static constexpr int64_t TIMESTAMP_MIN = 0;
...
public:
     ...
     * on this soteria node, and set to 0 for transactions that were created
...
private:
    ...
    bool SelectCoinsPQ(const std::vector<COutput>& vAvailableCoins, const CMutableTransaction& txNew, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl = nullptr) const; 
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetSOTER = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and SOTER
     */
