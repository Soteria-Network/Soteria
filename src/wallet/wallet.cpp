...
/**
 * Fees smaller than this (in soterio) are considered zero fee (for transaction creation)
...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetSOTER, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (const auto& entry : mapWallet) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (coinControl && !isAssetScript && coinControl->HasSelected() && !coinControl->fAllowOtherInputs && !coinControl->IsSelected(COutPoint(entry.first, i)))
                    continue;
                ...
                if (fGetSOTER) { // Looking for SOTER Tx OutPoints Only
                    if (fSOTERLimitHit) // We hit our limit
                        continue;

                    // We only want SOTER OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
/** This shows a performance hit & noticable overhead in tests so we will stay with BTC approach not with standard C++ std::mt19937
...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (size_t nIn = 0; nIn < tx.vin.size(); ++nIn) {
        ...
        if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, nHashType),
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-soteria-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** SOTER START */
                    // Check to see if you need to make an asset data outpoint OP_SOTER_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** SOTER START */ /** SOTER END */
                    {
