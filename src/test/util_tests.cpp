...
#undef E
    ...
    BOOST_AUTO_TEST_CASE(ParseClientVersion_test)
    {
        ...
        BOOST_CHECK(ParseClientVersion("/SOTERIA:1.1.0/", major, minor, revision));
        BOOST_CHECK_EQUAL(major, 1);
        BOOST_CHECK_EQUAL(minor, 1);
        BOOST_CHECK_EQUAL(revision, 0);

        // Test version with single comment
        BOOST_CHECK(ParseClientVersion("/SOTERIA:1.0.1(mycomment)/", major, minor, revision));
        BOOST_CHECK_EQUAL(major, 1);
        BOOST_CHECK_EQUAL(minor, 0);
        BOOST_CHECK_EQUAL(revision, 1);

        // Test version with multiple comments
        BOOST_CHECK(ParseClientVersion("/SOTERIA:1.2.3(comment1; comment2)/", major, minor, revision));
        BOOST_CHECK_EQUAL(major, 1);
        BOOST_CHECK_EQUAL(minor, 2);
        BOOST_CHECK_EQUAL(revision, 3);

        // Test version with complex comments
        BOOST_CHECK(ParseClientVersion("/SOTERIA:2.0.0(build123; debug; test)/", major, minor, revision));
        BOOST_CHECK_EQUAL(major, 2);
        BOOST_CHECK_EQUAL(minor, 0);
        BOOST_CHECK_EQUAL(revision, 0);

        // Test case insensitive patterns
        BOOST_CHECK(ParseClientVersion("/Soteria:1.1.0(test)/", major, minor, revision));
        BOOST_CHECK_EQUAL(major, 1);
        BOOST_CHECK_EQUAL(minor, 1);
        BOOST_CHECK_EQUAL(revision, 0);
        ...
        BOOST_CHECK(!ParseClientVersion("/UNKNOWN:1.1.0/", major, minor, revision));
        BOOST_CHECK(!ParseClientVersion("/SOTERIA:invalid/", major, minor, revision));
        BOOST_CHECK(!ParseClientVersion("/SOTERIA:1.1/", major, minor, revision));
        BOOST_CHECK(!ParseClientVersion("/SOTERIA:1.1.abc/", major, minor, revision));

        // Test version checking function
        // Test that versions >= 1.0.0 are not below minimum
        BOOST_CHECK(!IsClientVersionBelowMinimum("/SOTERIA:1.0.0/"));
        BOOST_CHECK(!IsClientVersionBelowMinimum("/SOTERIA:1.0.0(comment)/"));
        BOOST_CHECK(!IsClientVersionBelowMinimum("/SOTERIA:1.1.0/"));
        BOOST_CHECK(!IsClientVersionBelowMinimum("/SOTERIA:2.0.0(build; test)/"));

        // Test that versions < 1.0.0 are below minimum
        BOOST_CHECK(IsClientVersionBelowMinimum("/SOTERIA:0.9.9/"));
        BOOST_CHECK(IsClientVersionBelowMinimum("/SOTERIA:0.9.9(comment)/"));
        BOOST_CHECK(IsClientVersionBelowMinimum("/SOTERIA:0.1.0/"));
