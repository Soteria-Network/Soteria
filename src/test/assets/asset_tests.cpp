...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of SOTERIACOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("SOTER", type));
        BOOST_CHECK(!IsAssetNameValid("SOTERIA", type));
        
        //- Versions of SOTERIACOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("SOTERIA.COIN", type));
        BOOST_CHECK(IsAssetNameValid("SOTERIA_COIN", type));
        BOOST_CHECK(IsAssetNameValid("SOTERSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERSOTER", type));
        BOOST_CHECK(IsAssetNameValid("SOTERIASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDESOTERIA", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_SOTERIAS", type));
        BOOST_CHECK(IsAssetNameValid("SESOTEROT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("SOTERIA", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("SOTERIA", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
