...
static constexpr int32_t MAX_PEER_TX_IN_FLIGHT = 200;
/**
...
static constexpr int64_t GETDATA_TX_INTERVAL = 20 * 1000000;
...
static constexpr uint64_t RANDOMIZER_ID_ADDRESS_RELAY = 0x3cac0035b5866b90ULL; // SHA256("main address relay")[0:8]
...
bool ParseClientVersion(const std::string& userAgent, int& major, int& minor, int& revision) {
    ...
    // Look for various SOTERIA version patterns (case insensitive)
    size_t pos = std::string::npos;
    size_t skip_len = 0;
    
    // Try different patterns
    if ((pos = userAgent.find("SOTERIA:")) != std::string::npos) {
        skip_len = 8; // Skip "SOTERIA:"
    } else if ((pos = userAgent.find("Soteria:")) != std::string::npos) {
        skip_len = 8; // Skip "Soteria:"
    } else if ((pos = userAgent.find("soteria:")) != std::string::npos) {
        skip_len = 8; // Skip "soteria:"
    } else if ((pos = userAgent.find("Soteria-seeder:")) != std::string::npos) {
        skip_len = 15; // Skip "Soteria-seeder:"
    } else if ((pos = userAgent.find("SOTERIA-SEEDER:")) != std::string::npos) {
        skip_len = 15; // Skip "SOTERIA-SEEDER:"
    } else if ((pos = userAgent.find("soteria-seeder:")) != std::string::npos) {
        skip_len = 15; // Skip "soteria-seeder:"
...
static bool TipMayBeStale(const Consensus::ConsensusParams& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    AssertLockHeld(cs_main);
...
// Requires cs_main
static bool CanDirectFetch(const Consensus::ConsensusParams& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::ConsensusParams& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    if (count == 0)
        return;
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
 ...
 strprintf("Expected to offer services %08x", GetDesirableServiceFlags(nServices))));
        ...
        if (IsDualAlgoEnabled(chainActive.Tip(), Params().GetConsensus()) && nVersion < DUALALGO_VERSION)
 {
            LogPrintf("peer=%d using obsolete version; missing soterC; %i; disconnecting\n", pfrom->GetId(), nVersion);
            connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", DUALALGO_VERSION)));
            pfrom->fDisconnect = true;
            return false;
        }
 ...
 strprintf("Version must be %d or greater", ASSETS_VERSION)));
        ...
        // Decide how to handle alert migration in Soteria Network and either make this a Soteria
        // alert or re-introduce the system that removed from Bitcoin upstream
        //  TODO: Replace this with a valid Soteria Network alert
        // Disabled meantime as the remote client will consider the nonsense alert a hack and drops the connection
/*        if (pfrom->nVersion <= 70012) {
            CDataStream finalAlert(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
            connman->PushMessage(pfrom, CNetMsgMaker(nSendVersion).Make("alert", finalAlert));
        } this actually removed in reputable blockchains, cause it is an old practice nowadays */
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        // Assuming vAddr is a vector of CAddress
   ...
   std::shuffle(vAddr.begin(), vAddr.end(), eng);
