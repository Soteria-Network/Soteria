...
UniValue UpdateAddressTag(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria address: ") + address);
    }
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria change address: ") + change_address);
           }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "9. \"has_ans\"               (boolean, optional, default=false), whether SNS data is going to be added to the asset\n"
            "10. \"ans_id\"               (string, optional but required if has_ans = 1), an SNS ID once messaging is activated\n"
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) SOTER will be burned for each asset created.\n"
                ...
                "4. \"ans_ids\"               (array, optional) SNS IDs corresponding to each supplied tag (should be same size as \"asset_tags\")\n"
                "5. \"to_address\"            (string, optional, default=\"\"), address assets will be sent to, if it is empty, address will be generated for you\n"
                "6. \"change_address\"        (string, optional, default=\"\"), address the the soter change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!ansIDS.isNull()) {
        if (!ansIDS.isArray() || ansIDS.size() != assetTags.size()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("If provided, SNS IDs must be an array of the same size as the asset tags array."));
        }
    }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a soteria address\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria address: ") + to_address);
    }
    ...
    std::string soter_change_address = "";
    if (request.params.size() > 5) {
        soter_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination soter_change_dest = DecodeDestination(soter_change_address);
    if (!soter_change_address.empty() && !IsValidDestination(soter_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("SOTER change address must be a valid address. Invalid address: ") + soter_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = soter_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "7. \"soter_change_address\"       (string, optional, default = \"\") the transactions SOTER change will be sent to this address\n"
    ...
    std::string soter_change_address = "";
    if (request.params.size() > 6) {
        soter_change_address = request.params[6].get_str();
    }
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid SOTER address: ") + address);
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria address: ") + change_address);
        }
    }
...
UniValue ansencode(const JSONRPCRequest& request)
{
    ...
    CSoteriaNameSystem::Type type;

    // Set type
    if (strType == "addr") type = CSoteriaNameSystem::ADDR;
    else if (strType == "ip") type = CSoteriaNameSystem::IPv4;
    ...
    formattedTypeData = CSoteriaNameSystem::FormatTypeData(type, strData, error);

    if(error != "") throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("SNS error: ") + error);

    // Check strData (sanity check, typeData already checked in FormatTypeData)
    if (!CSoteriaNameSystem::CheckTypeData(type, formattedTypeData)) throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid SNS data: ") + strData);
...
UniValue ansdecode(const JSONRPCRequest& request)
{
    ...
    if (!CSoteriaNameSystem::IsValidID(strID)) throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid SNS ID: ") + strID);
    ...
    if(ansID.type() == CSoteriaNameSystem::ADDR) {
        result.pushKV("ans_addr", ansID.addr());
    } else if(ansID.type() == CSoteriaNameSystem::IPv4) {
