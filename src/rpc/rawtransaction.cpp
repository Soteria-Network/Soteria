...
{
    // Call into TxToUniv() in soteria-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in soteria-common, so we query them here and push the
        ...
        if (!(tx.IsCoinBase())) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    in.pushKV("value", ValueFromAmount(spentInfo.soterios));
                    in.pushKV("valueSat", spentInfo.soterios);
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CSoteriaAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CSoteriaAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Outputs are addresses (paired with a SOTER amount, data or object specifying an asset operation) or data.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nPaying for Asset Operations:\n"
            "  Some operations require an amount of SOTER to be sent to a burn address:\n"
            "\n"
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination soteria address.\n"
            "                                               Each output must have a different address.\n"
            "         x.xxx                             (number or string, required) The SOTER amount\n"
            "           or\n"
            ...
            "               \"ans_id\":\"id\"          (string, optional) an SNS ID to attach Soteria Name System data\n"
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/SoteriaNetwork/Soteria/issues/675
   //TODO"               \"custom_owner_address\": \"addr\" (string, optional) owner token will get sent to this address if set\n"
            ...
            "               \"ans_ids\":[\"hash\", ...],     (array, optional) SNS IDs corresponding to each supplied tag \n"
...
// TODO, if we decide to remove the consensus check https://github.com/SoteriaNetwork/Soteria/issues/675, remove or add the code (requires consensus change)
...
//                    }
                ...
                else if (assetKey_ == "reissue")
                {
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, owner_change_address is not a valid Soteria address");
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER,
                                           "Invalid parameter, owner_change_address is not a valid Soteria address");
                    ...
                    if (fHasRootChange && !IsValidDestinationString(root_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, root_change_address is not a valid Soteria address");
                    ...
                    for (int i = 0; i < (int)addresses.size(); i++) {
                        if (!IsValidDestinationString(addresses[i].get_str()))
                            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, supplied address is not a valid Soteria address");
                    }
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) soteria address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"hasANS\": true|false,    (boolean) If this asset has an SNS ID. (Only appears in type (new_asset if hasANS is true))\n"
            "  \"ans_id\": \"hash\",      (string) The SNS ID for the new asset. (Only appears in type (new_asset))\n"
            "  \"new_ans_id\":\"hash\",    (string) If new SNS ID (Only appears in type. (reissue_asset))\n"
            "}\n"
            "\nExamples:\n"
