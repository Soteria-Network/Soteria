...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    uint32_t nShift = (blockindex->nBits >> 24) & 0xff;
    double dDiff =
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    if (spentInfo.addressType == 1) {
                        delta.push_back(Pair("address", CSoteriaAddress(CKeyID(spentInfo.addressHash)).ToString()));
                    } else if (spentInfo.addressType == 2)  {
                        delta.push_back(Pair("address", CSoteriaAddress(CScriptID(spentInfo.addressHash)).ToString()));
                    } else {
                        continue;
                    }
                    delta.push_back(Pair("soterios", -1 * spentInfo.soterios));
        ...
        for (unsigned int k = 0; k < tx.vout.size(); k++) {
            ...
            if (out.scriptPubKey.IsPayToScriptHash()) {
                ...
                delta.push_back(Pair("address", CSoteriaAddress(CScriptID(uint160(hashBytes))).ToString()));

            } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                std::vector<unsigned char> hashBytes(out.scriptPubKey.begin() + 3, out.scriptPubKey.begin() + 23);
                delta.push_back(Pair("address", CSoteriaAddress(CKeyID(uint160(hashBytes))).ToString()));
            } else {
                continue;
            }

            delta.push_back(Pair("soterios", out.nValue));
...
UniValue getblockstats(const JSONRPCRequest& request)
{
    ...
    for (size_t txIndex = 0; txIndex < block.vtx.size(); txIndex++) {
        ...
        if (tx->IsCoinBase()) {
            ...
            for (const auto& out : tx->vout) {
                subsidy += out.nValue;
                utxo_size_inc += ::GetSerializeSize(out, SER_NETWORK, PROTOCOL_VERSION);
            }
            total_out += subsidy;
        } else {
            ...
            if (hasUndoData && undoIndex < blockUndo.vtxundo.size()) {
                ...
                utxo_size_inc -= ::GetSerializeSize(prevout.out, SER_NETWORK, PROTOCOL_VERSION);
         }
            ...
            for (const auto& output : tx->vout) {
                ...
                total_outs++;
                utxo_size_inc += ::GetSerializeSize(output, SER_NETWORK, PROTOCOL_VERSION);
            }
    ...
    stats.push_back(Pair("utxo_size_inc", utxo_size_inc));
...
static UniValue getdifficulty(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"powalgo\":\"xxxx\"     (string, optional) This can be set to \"soterg\" or \"soterc\" or \"soterhash\" or \"X8S\". If omitted, wallet's default is assumed (-powalgo conf option)\n"
            "\nResult:\n"
    ...
    if (!IsDualAlgoEnabled(chainActive.Tip(), Params().GetConsensus()) && powType != POW_TYPE_SOTERG)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Non soterg algo requested but Dual_Algo not enabled");
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of Soteria addresses\n"
...
static void BIP9SoftForkDescPushBack(UniValue& bip9_softforks, const std::string& name, const Consensus::ConsensusParams& consensusParams, Consensus::DeploymentPos id)
{
