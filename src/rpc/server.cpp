...
CAmount AmountFromValue(const UniValue& value, bool p_isSOTER)
{
    ...
    if (p_isSOTER && !MoneyRange(amount))
...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    return "Soteria server stopping";
}
...
{
    return "> soteria-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:8323/\n";
}
...
void CheckIPFSTxidMessage(const std::string &message, int64_t expireTime)
{
    ...
    // Start SNS checks
    bool fHasANS = (msglen >= CSoteriaNameSystem::prefix.size() + 1) && (message.substr(0, CSoteriaNameSystem::prefix.length()) == CSoteriaNameSystem::prefix) && (msglen <= 64);
    if (fHasANS && !IsSoteriaNameSystemDeployed()) {
        throw JSONRPCError(RPC_INVALID_PARAMS, std::string("SNS IDs not allowed when they are not deployed."));
    }

    if (fHasANS && !CSoteriaNameSystem::IsValidID(message)) {
        throw JSONRPCError(RPC_INVALID_PARAMS, std::string("Invalid SNS ID"));
    }
    // End SNS checks
