...
static UniValue GetNetworkHashPS(int lookup, int height, POW_TYPE powType) {
    ...
    // If we have stepped back to (or started looking up from) pre soterC, but requested soterC pow type, then there are no hashes
...
static UniValue getnetworkhashps(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "3. powalgo     (string, optional) This can be set to \"soterg\". If omitted, wallet's default is assumed (-powalgo conf option)\n" 
            "\nResult:\n"
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated soteria to.\n"
...
static UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"difficulty\": xxx.xxxxx    (numeric) The current difficulty for pre-Dual_Algo\n"
            "  \"difficulty_algorithm\": x.x (numeric) the current difficulty for Dual_Algo once activated per algorithm\n"
            "  \"networkhashps\": nnn,      (numeric) The network hashes per second\n"
            "  \"hashespersec\": nnn,       (numeric) The hashes per second of built-in miner\n"
            "  \"pooledtx\": n              (numeric) The size of the mempool\n"
            "  \"chain\": \"xxxx\",           (string) current network name as defined in BIP70 (main, test, regtest)\n"
            "  \"warnings\": \"...\"          (string) any network and blockchain warnings\n"
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when soteriad is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
// NOTE: Unlike wallet RPC (which use SOTER values), mining RPCs follow GBT (BIP 22) in using soterio amounts
static UniValue prioritisetransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "3. fee_delta      (numeric, required) The fee value (in soterios) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"powalgo\":\"xxxx\"     (string, optional) This can be set to \"soterg\". If omitted, wallet's default is assumed (-powalgo conf option)\n"
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                    (numeric) difference in value between transaction inputs and outputs (in soterios); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "  },\n"
            "  \"coinbasevalue\" : n,              (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in soterios)\n"
            "  \"FoundationReserveAddress\" : n, (string) Foundation Reserve Address\n"
            "  \"FoundationReserveValue\" : n,   (numeric) Foundation Reserve Value, 10% of the coinbase\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Soteria is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Soteria is downloading blocks...");
...
// Create new block, Get mining address if it is set
...
if (!address.empty()) {
  ...
  throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid -miningaddress is not a valid address. Please use a valid address");
    }
    
} else {
       //script = CScript() << OP_TRUE;
       // Remove OP_TRUE fallback since we now require an address, security risk
  throw JSONRPCError(RPC_INVALID_PARAMETER, "You must specify -miningaddress");
}

pblocktemplate = BlockAssembler(Params()).CreateNewBlock(script, fSupportsSegwit, powType); // Dual algo: Include powType       
...
/*  UniValue founderObj(UniValue::VOBJ);
    ...
    if(pblock->txoutFounder!= CTxOut()) {
    	...
    	CSoteriaAddress address2(address);
...
*/ // TODO
    ...
    result.push_back(Pair("FoundationReserveAddress", Params().FoundationReserveAddress()));
    ...
    result.push_back(Pair("weightlimit", (int64_t)GetMaxBlockWeightForPrev(pindexPrev, Params().GetConsensus())));
    }
...
//    if (block.nHeight > (uint32_t)chainActive.Height() + 4)
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(SOTERIA_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateSoterias(fGenerate, nGenProcLimit, Params());
