...
bool DummySignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& keyid, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    if (IsUAHFenabledForCurrentBlock()) {
        nHashType |= SIGHASH_FORKID; // Now uses 0x0100
    }
    vchSig.assign(72, '\000');
    vchSig[0] = 0x30; // DER header
    vchSig[1] = 69; // Length
    vchSig[2] = 0x02; // Integer tag
    vchSig[3] = 33; // R length
    vchSig[4] = 0x01; // R value header
    vchSig[4 + 33] = 0x02; // Integer tag
    vchSig[5 + 33] = 32; // S length
    vchSig[6 + 33] = 0x01; // S value header
    vchSig[6 + 33 + 32] = nHashType & 0xff; // ForkID is 2-byte 
    return true;
}
