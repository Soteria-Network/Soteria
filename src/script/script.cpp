...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** SOTER START */
    case OP_SOTER_ASSET              : return "OP_SOTER_ASSET";
    /** SOTER END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_SOTER_ASSET) { // OP_SOTER_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == SOTER_S) { // Check to see if SOTER starts at 27 ( this->size() < 105)
                if ((*this)[28] == SOTER_W)
                    if ((*this)[29] == SOTER_R)
                        index = 30;
            } else {
                if ((*this)[28] == SOTER_S) // Check to see if SOTER starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == SOTER_W)
                        if ((*this)[30] == SOTER_R)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == SOTER_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == SOTER_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == SOTER_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == SOTER_S) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_SOTER_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_SOTER_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_SOTER_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_SOTER_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_SOTER_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_SOTER_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
