...
// Dump addresses to peers.dat and banlist.dat every 600s, default=900
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Soteria users should be well used to small numbers with many decimal places by now :)
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Soteria " + FormatFullVersion();
        try {
            while (true) {
#ifndef UPNPDISCOVER_SUCCESS
                /* miniupnpc 1.5 */
...
/** This patch is all about giving the node a little “kick-start” whenever its address database (addrman) is basically empty (fewer than 10 peers). Phicoin Project Implementation */
...
// Add fixed seed nodes immediately for faster startup
    ...
    if (!fixedSeedsAdded && addrman.size() < 10) {
        ...
        std::vector<CAddress> vFixedSeeds = convertSeed6(Params().FixedSeeds());
    ...
    while (!interruptNet)
    {
        ...
        if (addrman.size() < 5 && (GetTime() - nStart > 30)) { // add fixed seeds
            static bool done = false;
            if (!done) {
                LogPrintf("Adding fixed seed nodes as DNS still unavailable after 30 seconds.\n");
                CNetAddr local;
                local.SetInternal("fixedseeds");
...
if (connOptions.m_use_addrman_outgoing || !connOptions.m_specified_outgoing.empty()) {
    threadOpenConnections = std::thread(
        ...
        "opencon",
        std::function<void()>([this, specified_outgoing = connOptions.m_specified_outgoing]() {
            ThreadOpenConnections(specified_outgoing);
        })
    );
}
    // Process messages
...
bool CConnman::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    {
        // keep a large enough buffer to at least relay each block once, fastest block
        uint64_t timeLeftInCycle = GetMaxOutboundTimeLeftInCycle();
// for now we will ignore it until we know the lowest block time        
//     uint64_t buffer = timeLeftInCycle / Params().GetConsensus().nBlockTimeDivisor * GetMaxBlockSerializedSize();
    // Base divisor and max block size
    const auto& cp = Params().GetConsensus();
    uint64_t baseDivisor = cp.nBlockTimeDivisor;
    uint64_t maxBlkSize = GetMaxBlockSerializedSize();

    // Fixed-divisor headroom
    uint64_t headroom = (timeLeftInCycle / baseDivisor) * maxBlkSize;

    // If we’re in last 2 × baseDivisor seconds, add one extra block
    uint64_t threshold = baseDivisor * 2;                   // in the future we may change it to baseDivisor * 1
    if (timeLeftInCycle < threshold) {
        headroom += maxBlkSize;
    }

    uint64_t buffer = headroom;
