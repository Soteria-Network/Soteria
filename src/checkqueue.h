...
#ifndef SOTERIA_CHECKQUEUE_H
#define SOTERIA_CHECKQUEUE_H
#include <sync.h>
#include <atomic>
#include <algorithm>
#include <vector>
...
private:
    ...
    bool Loop(bool fMaster = false)
    {
        ...
        {
            {
                ...
                while (queue.empty()) {
                    if ((fMaster || fQuit.load(std::memory_order_acquire)) && nTodo == 0) { // Atomic load
                        nTotal--;
                        bool fRet = fAllOk;
        ...
        return false;  // Secondary exit point (shutdown case)
    }
...
public:
    ...
    ~CCheckQueue()
    {
    Shutdown();  // Safe shutdown in destructor
    }
