...
/* Block Addresses
static const std::vector<std::string> Blacklist_Addresses = {
    "address1",
    "address2"
}; */

// static const int Blacklist_Height = 666;
      ...
      /** Blacklist Addresses
         if (chainActive.Tip() && chainActive.Tip()->nHeight + 1 >= Blacklist_Height) {
             ...
             if (ExtractDestination(txout.scriptPubKey, dest)) {
               ...
               if (std::find(Blacklist_Addresses.begin(), Blacklist_Addresses.end(), address) != Blacklist_Addresses.end()) {
                     return state.DoS(100, false, REJECT_INVALID, "This address is already blacklisted.");
                 }
             }
         }  */
        
        /** SOTER START */
        // Find and handle all new OP_SOTER_ASSET null data transactions
...
/** (DER sig 71–73 bytes + pubkey 33 bytes (compressed) + push ops and sighash byte: ~3 bytes), Total ~110 bytes, exceeding 100. Uncompressed keys, extra data, or future script types can push scriptSig up to 150–170 bytes. 200 bytes ensures that all valid P2PKH transactions—whether they use uncompressed pubkeys or slightly oversized DER signatures—pass policy checks. */
/** The coin using a 250-byte cap wants to accept more complex unlocking scripts by default—think P2SH-wrapped multisig (multiple signatures plus the redeem script), CLTV/CSV wrappers, cross-chain atomic-swap scripts, or tiny bits of metadata embedded in the scriptSig. Avoiding false rejections of legitimate transactions that naturally exceed smaller thresholds (especially multisig or advanced script templates). */

    if (tx.IsCoinBase())
    {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 250)
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_SOTER_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_SOTER_ASSET)) {
                    if (out.scriptPubKey[0] != OP_SOTER_ASSET) {
                        return state.DoS(100, false, REJECT_INVALID,
                                         "bad-txns-op-soter-asset-not-in-right-script-location");
                    }
                }
            }
        }
...
/**        if (nSpendHeight >= Blacklist_Height) {
            ...
            if (ExtractDestination(coin.out.scriptPubKey, inputDest)) {
               ...
               if (std::find(Blacklist_Addresses.begin(), Blacklist_Addresses.end(), addr) != Blacklist_Addresses.end()) {
                  return state.DoS(100, false, REJECT_INVALID, "Transaction Contains An Address From Blacklisted Addresses");
                }
            }
        } */
    }
                ...
                if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                    ...
                    if (out.scriptPubKey.Find(OP_SOTER_ASSET)) {
                        if (AreRestrictedAssetsDeployed()) {
                            if (out.scriptPubKey[0] != OP_SOTER_ASSET) {
                                return state.DoS(100, false, REJECT_INVALID,
                                                 "bad-txns-op-soter-asset-not-in-right-script-location", false, "", tx.GetHash());
                            }
                        } else {
