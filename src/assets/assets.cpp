...
// excluding owner tag ('!')
static const auto MAX_NAME_LENGTH = 128; // default=31
...
static const std::regex SOTERIA_NAMES("^RVN$|^SOTER$|^SOTERIA$|^SOTERIA$|^#SOTER$|^#SOTERIA$|^#SOTERIA$");
...
bool IsRootNameValid(const std::string& name)
{
    return std::regex_match(name, ROOT_NAME_CHARACTERS)
        ...
        && !std::regex_match(name, SOTERIA_NAMES);
}
...
bool IsSubNameValid(const std::string& name)
{
   ...
   // DNS RFC compliant validation - position restrictions are built into the regex
    ...
    return !std::regex_match(name, DOUBLE_PUNCTUATION)
           && !std::regex_match(name, LEADING_PUNCTUATION)
           && !std::regex_match(name, TRAILING_PUNCTUATION);        
}
...
bool IsTypeCheckNameValid(const AssetType type, const std::string& name, std::string& error)
{
    if (type == AssetType::UNIQUE) {
        ...
        if (!valid && IsAssetNameASubasset(name) && name.size() < MIN_ASSET_LENGTH) { error = "Name must have at least 1 characters (Valid characters are: A-Z 0-9 _ .)";  return false; }
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SOTER_S); // s
    vchMessage.push_back(SOTER_W); // w
    vchMessage.push_back(SOTER_R); // r
    vchMessage.push_back(SOTER_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_SOTER_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
void CNewAsset::ConstructOwnerTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SOTER_W); // w
    vchMessage.push_back(SOTER_R); // r
    vchMessage.push_back(SOTER_O); // o
...
{
    // Must contain at least 3 outpoints (SOTER burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
bool CTransaction::VerifyNewAsset(std::string& strError) const {
    // Issuing an Asset must contain at least 3 CTxOut( Soteria Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Issuing an Asset must contain at least 2 CTxOut( Soteria Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedAsset(std::string& strError) const {
    // Issuing a restricted asset must cointain at least 4 CTxOut(Soteria Burn Tx, Asset Creation, Root Owner Token Transfer, and CNullAssetTxVerifierString)
    if (vout.size() < 4) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Soteria Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
void CAssetTransfer::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SOTER_W); // w
    vchMessage.push_back(SOTER_R); // r
    vchMessage.push_back(SOTER_T); // t
...
void CReissueAsset::ConstructTransaction(CScript& script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.push_back(SOTER_R); // s
    vchMessage.push_back(SOTER_W); // w
    vchMessage.push_back(SOTER_R); // r
    vchMessage.push_back(SOTER_S); // s
...
bool CAssetsCache::DumpCacheToDatabase()
{
    try {
        ...
        // flush accumulated asset DB operations
...
{
    // Check the first transaction and verify that the correct SOTER Amount
...
{
    // Verify 'soterq' is in the transaction
...
{
    // Verify 'soterr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
{
    return CSoteriaNameSystem::DecodeHex(decoded);
}

// SNS (ID to hex)
std::string DecodeANS(std::string encoded)
{
    CSoteriaNameSystem ansID(encoded);
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the SOTER required by the burnAmount
...
bool CreateReissueAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueAsset& reissueAsset, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soteria address: ") + address);
        return false;
    }
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any SOTER, transfering an asset requires a network fee"));
        return false;
    }
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_SOTER_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_SOTER_ASSET OP_SOTER_ASSET OP_SOTER_ASSETS data transaction to the transaction
...
bool CheckANS(const std::string& ansID, std::string& strError) {
    if(CSoteriaNameSystem::IsValidID(EncodeANS(ansID))) return true;
    strError = _("Invalid parameter: Soteria Name System ID is not valid");
    return false;
}
...
void CNullAssetTxData::ConstructGlobalRestrictionTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_SOTER_ASSET << OP_RESERVED << OP_RESERVED << ToByteVector(vchMessage);
}
...
void CNullAssetTxVerifierString::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_SOTER_ASSET << OP_RESERVED << ToByteVector(vchMessage);
}
...
bool ContextualCheckNewAsset(CAssetsCache* assetCache, const CNewAsset& asset, std::string& strError, bool fCheckMempool)
{
    ...
    // SNS not allowed when they are not deployed
    if (asset.nHasANS && !IsSoteriaNameSystemDeployed()) {
        strError = _("Invalid parameter: SNS IDs not allowed when they are not deployed.");
        return false;
    }

    // Check asset name for SNS
    if (IsAssetNameARoot(asset.strName) && asset.nHasANS) {
        bool shortLength = asset.strName.length() <= CSoteriaNameSystem::domain.length();
        if (shortLength || asset.strName.substr(asset.strName.length() - CSoteriaNameSystem::domain.length()) != CSoteriaNameSystem::domain) {
            strError = std::string(_("Invalid parameter: asset name needs to end in '")) + CSoteriaNameSystem::domain + std::string(_("' since SNS data is attached."));
            return false;
        }
    }
...
bool ContextualCheckReissueAsset(CAssetsCache* assetCache, const CReissueAsset& reissue_asset, std::string& strError, const CTransaction& tx)
{
    ...
    // SNS not allowed when they are not deployed
    if (reissue_asset.strANSID != "" && !IsSoteriaNameSystemDeployed()) {
        strError = _("Invalid parameter: SNS IDs not allowed when they are not deployed.");
        return false;
    }
    ...
    if (IsAssetNameARoot(reissue_asset.strName) && reissue_asset.strANSID != "") {
        bool shortLength = reissue_asset.strName.length() <= CSoteriaNameSystem::domain.length();
        if (shortLength || reissue_asset.strName.substr(reissue_asset.strName.length() - CSoteriaNameSystem::domain.length()) != CSoteriaNameSystem::domain) {
            strError = std::string(_("Invalid parameter: asset name needs to end in '")) + CSoteriaNameSystem::domain + std::string(_("' since SNS data is attached."));
            return false;
        }
    }
