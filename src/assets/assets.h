...
#define SOTERIA_ASSET_PROTOCOL_H
...
bool CheckIssueDataTx(const CTxOut& txOut); // OP_SOTERIA_ASSET SOTERQ (That is a Q as in Que not an O)
bool CheckOwnerDataTx(const CTxOut& txOut);// OP_SOTERIA_ASSET SOTERO
bool CheckReissueDataTx(const CTxOut& txOut);// OP_SOTERIA_ASSET SOTERR
bool CheckTransferOwnerTx(const CTxOut& txOut);// OP_SOTERIA_ASSET SOTERT
...
//! Decode and Encode IPFS hashes, SNS IDs, or OIP hashes
...
#endif //SOTERIA_ASSET_PROTOCOL_H
