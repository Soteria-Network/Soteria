...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SOTER
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SOTER") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SOTER with divisor %d\n", __func__, srcUnitDivisor);
    }
    ...
    for (auto & ownership : nonExceptionOwnerships) {
        ...
        // Caculate the reward with potentional unit inaccurancies e.g with units 4, 90054100 soterios = 0.90054100
        ...
        // Replace all none accurate units back with zeros e.g with units 4 9005 => 90050000 soterios = 0.90050000
        rewardAmt *= static_cast<CAmount>(pow(10, COIN_DIGITS_PAST_DECIMAL - distributionAsset.units));
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SOTER differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SOTER") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Soteria address (already validated during ownership snapshot creation)
