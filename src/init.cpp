...
// SOTER START, Dummy implementation of the WalletInitInterface, to keep all the wallet‐disabled logic in one spot
#if !(ENABLE_WALLET)
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateSoterias(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SOTERIA_CONF_FILENAME));
    if (mode == HMM_SOTERIAD) {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SOTERIA_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
 ...
 //          "\n" +
   ...
   return CopyrightHolders(strprintf(_("Copyright (C) %i"), COPYRIGHT_YEAR) + " ") + "\n" +
           ...
           "\n" +
           "\n" +
           _("This is the official software for Soteria Network Wallet.") + "\n" +
           strprintf(_("Distributed under the MIT software license, see the accompanying file %s or %s"), "COPYING", "<https://opensource.org/licenses/MIT>") + "\n" +
           "\n" +
           strprintf(_("The Soteria Network Wallet includes software developed by the OpenSSL Project for use in the OpenSSL Toolkit %s and cryptographic software written by Eric Young."), "<https://www.openssl.org>") +
           "\n";
}
...
/** Sanity checks
 *  Ensure that Soteria is running in a usable environment with all
...
//fRequireStandard = !gArgs.GetBoolArg("-acceptnonstdtxn", !chainparams.RequireStandard()); // this can bring security issue
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Soteria process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SOTERIA_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

   // Warn about relative path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " 
                  "current working directory '%s'. This is fragile, because if Soteria Network is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Soteria Network is started while in a temporary directory.\n",
    ...
    GenerateSoterias(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
