...
class CSoteriaAddressVisitor : public boost::static_visitor<bool>
{
private:
    CSoteriaAddress* addr;

public:
    explicit CSoteriaAddressVisitor(CSoteriaAddress* addrIn) : addr(addrIn) {}

    bool operator()(const CKeyID& id) const { return addr->Set(id); }
    bool operator()(const CScriptID& id) const { return addr->Set(id); }
    bool operator()(const CNoDestination& no) const { return false; }
    bool operator()(const WitnessV2PQDestination&) const { return false; } // PQ uses bech32m
};

} // namespace

bool CSoteriaAddress::Set(const CKeyID& id)
{
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CSoteriaAddress::Set(const CScriptID& id)
{
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CSoteriaAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(CSoteriaAddressVisitor(this), dest);
}

bool CSoteriaAddress::IsValid() const
{
    return IsValid(Params());
}

bool CSoteriaAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination CSoteriaAddress::Get() const
{
    if (!IsValid())
...
bool CSoteriaAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
void CSoteriaSecret::SetKey(const CKey& vchSecret)
{
...
bool CSoteriaSecret::IsValid() const
{
...
bool CSoteriaSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CSoteriaSecret::SetString(const std::string& strSecret)
{
...
CTxDestination DecodeDestination(const std::string& str)
{
    ...
    // Fall back to base58
    return CSoteriaAddress(str).Get();
}
...
bool IsValidDestinationString(const std::string& str, const CChainParams& params)
{
    ...
    return CSoteriaAddress(str).IsValid(params);
}
