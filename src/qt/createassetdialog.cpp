...
void CreateAssetDialog::setUpValues()
{
    ...
    QStringList list;
    list.append(tr("Main Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::ROOT)) + ")");
    list.append(tr("Sub Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB)) + ")");
    list.append(tr("Unique Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::UNIQUE)) + ")");
    list.append(tr("Messaging Channel Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::MSGCHANNEL)) + ")");
    list.append(tr("Qualifier Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::QUALIFIER)) + ")");
    list.append(tr("Sub Qualifier Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB_QUALIFIER)) + ")");
    list.append(tr("Restricted Asset") + " (" + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::RESTRICTED)) + ")");
    ...
    for (const auto type : ANSTypes)
        listTypes.append(QString::fromStdString(CSoteriaNameSystem::enum_to_string(type).first));
...
{
    /** Create the shadow effects on the frames */
    ui->frameFee->setStyleSheet(QString(".QFrame#frameFee { border-top: 2px solid %1;padding-top: 20px}").arg(platformStyle->Soteria_2B737F().name()));
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    {
        ui->labelBalance->setText(SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
bool CreateAssetDialog::checkIPFSHash(QString hash)
{
    ...
    if (!hash.isEmpty()) {
        std::string error;
        // Do not allow SNS in IPFS
        bool isANS = (hash.toStdString().substr(0, CSoteriaNameSystem::prefix.length()) == CSoteriaNameSystem::prefix);
...
void CreateAssetDialog::CheckFormState()
{
    ...
    if (!(ui->addressText->text().isEmpty() || IsValidDestination(dest)) && assetNameValid) {
        ...
        showMessage(tr("Warning: Invalid Soteria address"));
        return;
    }
    ...
    if (ui->ansBox->isChecked() && !ui->ansText->text().isEmpty()) {
        CSoteriaNameSystem::Type type = static_cast<CSoteriaNameSystem::Type>(ui->ansType->currentIndex());

        std::string error;
        std::string formattedTypeData;
        std::string typeData = ui->ansText->text().toStdString();
        
        formattedTypeData = CSoteriaNameSystem::FormatTypeData(type, typeData, error);
        ...
        CSoteriaNameSystem sns(type, formattedTypeData);

        if (!IsSoteriaNameSystemDeployed()) {
            ui->ansText->setStyleSheet("border: 2px solid red");
            showMessage(tr("SNS not deployed yet."));
            disableCreateButton();
            return;
        }

        if (!CSoteriaNameSystem::IsValidID(sns.to_string())) {
            ui->ansText->setStyleSheet("border: 2px solid red");
            showMessage(tr("Invalid SNS data."));
            disableCreateButton();
            return;
        } else {
            ui->ansText->setStyleSheet("");
...
void CreateAssetDialog::onANSTypeChanged(int index) {
    CSoteriaNameSystem::Type type = static_cast<CSoteriaNameSystem::Type>(index);
    ui->ansText->setPlaceholderText(QString::fromStdString(CSoteriaNameSystem::enum_to_string(type).second));
    ui->ansText->clear();
}
...
void CreateAssetDialog::onCreateAssetClicked()
{
    ...
    if (hasANS) {
        ...
        std::string formattedTypeData;
        CSoteriaNameSystem::Type type = static_cast<CSoteriaNameSystem::Type>(ui->ansType->currentIndex());
        formattedTypeData = CSoteriaNameSystem::FormatTypeData(type, ui->ansText->text().toStdString(), error);

        CSoteriaNameSystem ansID(type, formattedTypeData);
        ansDecoded = ansID.EncodeHex();

        // Warn user
        QMessageBox::critical(this, "SNS Warning", tr("Storing data using the Soteria Name System will forever stay in the blockchain. You can edit the SNS ID only if the asset is reissueable.") + QString("\n\nANS ID: ") + QString::fromStdString(ansID.to_string()), QMessageBox::Ok, QMessageBox::Ok);
    }
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetBurnAmount(type), 8)) + " SOTER";
    amount.append("</b>");
    ...
    if(nFeeRequired > 0)
    {
        ...
        questionString.append(SoteriaUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nFeeRequired));
    ...
    for (SoteriaUnits::Unit u : SoteriaUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(SoteriaUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
                                  .arg(SoteriaUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void CreateAssetDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void CreateAssetDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Soteria address"));
        }
        else // Valid address
        {
...
void CreateAssetDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void CreateAssetDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
                SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetRequiredFee(1000)) + "/kB")
        );
}
