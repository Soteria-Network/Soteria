...
const QString SoteriaGUI::DEFAULT_WALLET = "~Default";

static bool ThreadSafeMessageBox(SoteriaGUI* gui, const std::string& message, const std::string& caption, unsigned int style);

SoteriaGUI::SoteriaGUI(const PlatformStyle* _platformStyle, const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
    enableWallet(false),
...

{
    ...
    // Load fallback font in case Comic Sans is not available on the system
    ...
    QFont newFont("Comic Sans MS", 12);

   // Set new application font
...
void SoteriaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to an Soteria address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and soteria: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to SOTER addresses"));
    ...
    wrapAction->setStatusTip(tr("Wrapped Soteria"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Soteria addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Soteria addresses"));
    ...
    openAction->setStatusTip(tr("Open a soteria: URI or payment request"));

    importPrivateKeyAction = new QAction(platformStyle->TextColorIcon(":/icons/key"), tr("&Import Private Key..."), this);
    importPrivateKeyAction->setStatusTip(tr("Import a Soteria private key"));
...
/*    webWalletAction = new QAction(tr("&WebWallet"), this);
    ...
    openSmartcontractsAction->setMenuRole(QAction::NoRole);
    openSmartcontractsAction->setStatusTip(tr("Manage and create Soteria Smart Plans"));
    connect(openSmartcontractsAction, SIGNAL(triggered()), this, SLOT(showSmartcontracts()));
    /** SOTER END */
...
/*      QLabel* label = new QLabel();
        QImage soteria(":/icons/soteria");
        QImage soteriaScaled = soteria.scaled(70, 70, Qt::KeepAspectRatio);
        label->setPixmap(QPixmap::fromImage(soteriaScaled));
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/soteriatext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/soteria")));
        }
...
QString openSansFontString = "font: normal 22pt \"Open Sans\";";
        ...
        QString tbStyleSheet = ".QToolBar {background-color: transparent; border-color: transparent;} "
                               ...
                               ".QToolButton:checked {background: none; background-color: none; selection-background-color: #153144; color: %2; border: none; border-radius: 10px;} "
//                             ".QToolButton:hover {background: none; background-color: none; border: none; color: %3;} "
     ".QToolButton:hover {background: #153144; color: white; border: none; border-radius: 10px;} "                         
...
//
        QLabel* soteriaLabel = new QLabel("SOTERIA NETWORK");
        soteriaLabel->setAlignment(Qt::AlignCenter);
        soteriaLabel->setStyleSheet("color: cyan; background: none; font-weight: bold; font-size: 20px;"); // Adjust the font size
        
/*        QLabel* soterLabel = new QLabel("SOTER isn’t just another crypto.");
        soterLabel->setAlignment(Qt::AlignCenter);
        soterLabel->setStyleSheet("color: #05f7af; background: none; font-weight: bold; font-size: 18px;"); // Adjust the font size
 */       
       // Create a QLabel for version information
        ...
        versionLabel->setStyleSheet("color: cyan; background: none; font-weight: bold; font-size: 16px;"); // Adjust the font size
//
        ...
        QVBoxLayout* soteriaLabelLayout = new QVBoxLayout(toolbarWidget);
//      soteriaLabelLayout->addWidget(label);
        soteriaLabelLayout->addWidget(labelToolbar);
        soteriaLabelLayout->addItem(topSpacer);
//      soteriaLabelLayout->addWidget(toolbar);
        soteriaLabelLayout->addWidget(m_toolbar);
        soteriaLabelLayout->addItem(bottomSpacer);
        soteriaLabelLayout->addWidget(frameBlocks);

//        
//      soteriaLabelLayout->addWidget(soterLabel);
        soteriaLabelLayout->addWidget(soteriaLabel);
        soteriaLabelLayout->addWidget(versionLabel);  // Add the QLabel for version
//        
        
        soteriaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ...
        labelCurrentMarket->setText(tr("Soteria (SOTER) Market Price"));
        ...
        QLabel* labelBtcSOTER = new QLabel();
        labelBtcSOTER->setText("USDT / SOTER");
        labelBtcSOTER->setContentsMargins(15, 0, 0, 0);
        labelBtcSOTER->setFixedHeight(75);
        ...
        priceLayout->addWidget(labelBtcSOTER, 0, Qt::AlignVCenter | Qt::AlignLeft);
        ...
        QObject::connect(networkManager, &QNetworkAccessManager::finished,
            this, [=](QNetworkReply* reply) {
                ...
                QJsonObject soteriaNetwork = obj.value("soteria-network").toObject();
...
void SoteriaGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/soteriatext")));
        m_toolbar->setMaximumWidth(65);
...
void SoteriaGUI::setClientModel(ClientModel* _clientModel)
{
...
#ifdef ENABLE_WALLET
bool SoteriaGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool SoteriaGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void SoteriaGUI::removeAllWallets()
{
    if (!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SoteriaGUI::setWalletActionsEnabled(bool enabled)
{
...
void SoteriaGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void SoteriaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SoteriaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SoteriaGUI::optionsClicked()
{
...
void SoteriaGUI::showDebugWindow()
{
...
void SoteriaGUI::showDebugWindowActivateConsole()
{
...
/** SOTER START */
void SoteriaGUI::showSmartcontracts()
{
    smartcontracts->showNormal();
    smartcontracts->show();
    smartcontracts->raise();
    smartcontracts->activateWindow();
}
/** SOTER END */

#ifdef ENABLE_WALLET
void SoteriaGUI::openClicked()
{
...
void SoteriaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SoteriaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SoteriaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SoteriaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SoteriaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SoteriaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** SOTER START */
void SoteriaGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void SoteriaGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void SoteriaGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void SoteriaGUI::gotoRestrictedAssetsPage()
{
    restrictedAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoRestrictedAssetsPage();
};

void SoteriaGUI::gotoWrapPage()
{
...
void SoteriaGUI::updateNetworkState()
{
...
void SoteriaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SoteriaGUI::updateHeadersSyncProgressLabel()
{
...
void SoteriaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay) {
        if (header)
...
void SoteriaGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle = tr("Soteria"); // default title
...
void SoteriaGUI::changeEvent(QEvent* e)
{
...
void SoteriaGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void SoteriaGUI::showEvent(QShowEvent* event)
{
...
#ifdef ENABLE_WALLET
void SoteriaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "SOTER")
        msg += tr("Amount: %1\n").arg(SoteriaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(SoteriaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void SoteriaGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Transfer assets to SOTER addresses"));
...
void SoteriaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void SoteriaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void SoteriaGUI::showModalOverlay()
{
...
void SoteriaGUI::subscribeToCoreSignals()
{
...
void SoteriaGUI::unsubscribeFromCoreSignals()
{
...
void SoteriaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SoteriaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    setStyleSheet(QString("QLabel { color : %1; }").arg(platformStyle->Soteria_18A7B7().name()));
}
...
{
    menu = new QMenu(this);
    for (SoteriaUnits::Unit u : SoteriaUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(SoteriaUnits::name(u)), this);
        menuAction->setData(QVariant(u));
        menu->addAction(menuAction);
    }
    menu->setStyleSheet(QString("QMenu::item{ color: %1; } QMenu::item:selected{ color: %2; border: none;}").arg(platformStyle->Soteria_2B737F().name(), platformStyle->Soteria_2B737F().name()));
...
{
    setText(SoteriaUnits::name(newUnits));
}
