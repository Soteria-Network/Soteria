...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    l2->setText(SoteriaUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    ...
    l8->setText(SoteriaUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
...
void AssetControlDialog::updateView()
{
    ...
    for (const auto& coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SoteriaUnits::format(nDisplayUnit, nAmount));
