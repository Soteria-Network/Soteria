    ...
    mapper(0)
{
    ...
    QFont notoFont("Noto Sans Roboto", 12);
    ui->lang->setFont(notoFont);

    ui->soteriaAtStartup->setToolTip(ui->soteriaAtStartup->toolTip().arg(tr(PACKAGE_NAME)));
    ui->soteriaAtStartup->setText(ui->soteriaAtStartup->text().arg(tr(PACKAGE_NAME)));

    ui->openSoteriaConfButton->setToolTip(ui->openSoteriaConfButton->toolTip().arg(tr(PACKAGE_NAME)));
    ...
    ui->unit->setModel(new SoteriaUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->soteriaAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openSoteriaConfButton_clicked()
{
