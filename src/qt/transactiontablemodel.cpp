...
//{
//    columns[Amount] = SoteriaUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, SoteriaUnits::SeparatorStyle separators) const
{
    QString str;
    switch(wtx->type) {
        case TransactionRecord::Issue:
        ...
        default:
            {
            str = SoteriaUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DecorationRole:
    {
        ...
        return platformStyle->TextColorIcon(icon, COLOR_SOTERIA_2B737F);
    }
    case Qt::DisplayRole:
        switch (column) {
        case Status: return {};
        ...
        case Amount:
            return formatTxAmount(rec, true, SoteriaUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "SOTER")
               return QString::fromStdString(rec->assetName);
            else
               return QString(SoteriaUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, SoteriaUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, SoteriaUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "SOTER")
               ...
               assetName.append(QString(SoteriaUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case AssetName:
                return tr("The asset (or SOTER) removed or added to balance.");
            }
        }
    }
    return QVariant();
