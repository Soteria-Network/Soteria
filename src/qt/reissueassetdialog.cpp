...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + SoteriaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Soteria Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::buildUpdatedData()
{
    ...
    if (asset->nHasANS && (!ui->ansBox->isChecked() || (ui->ansBox->isChecked() && ui->ansText->text().isEmpty()))) {
        ...
        ansID = formatBlack.arg(tr("SNS ID"), ":", qstr) + "\n";
        ...
        CSoteriaNameSystem ansData(type, formattedTypeData);
        QString qstr = QString::fromStdString(ansData.to_string());
        ansID = formatGreen.arg(tr("SNS ID"), ":", qstr) + "\n";
    }
...
void ReissueAssetDialog::onAssetSelected(int index)
{
    ...
    if (index > 0) {
        ...
        if (asset->nHasANS) {
            QString qstr = QString::fromStdString(EncodeANS(asset->strANSID));
            assetdataANS = formatBlack.arg(tr("SNS ID"), ":", qstr) + "\n";
        }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " SOTER";
    amount.append("</b>");
