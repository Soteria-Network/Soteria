...
#include "guiutil.h"
#include <algorithm>
#include "soteriaaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Soteria address (e.g. %1)").arg(QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new SoteriaAddressEntryValidator(parent));
    widget->setCheckValidator(new SoteriaAddressCheckValidator(parent));
}
...
bool parseSoteriaURI(const QUrl& uri, SendCoinsRecipient* out)
{
    // return if URI is not valid or is no soteria: URI
    if (!uri.isValid() || uri.scheme() != QString("soteria"))
        return false;
...
bool parseSoteriaURI(QString uri, SendCoinsRecipient* out)
{
    // Convert soteria:// to soteria:
    //
    //    Cannot handle this later, because soteria:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("soteria://", Qt::CaseInsensitive)) {
        uri.replace(0, 10, "soteria:");
    }
    QUrl uriInstance(uri);
    return parseSoteriaURI(uriInstance, out);
}

QString formatSoteriaURI(const SendCoinsRecipient& info)
{
    QString ret = QString("soteria:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(SoteriaUnits::format(SoteriaUnits::SOTER, info.amount, false, SoteriaUnits::separatorNever));
        paramCount++;
    }
...
bool openSoteriaConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(SOTERIA_CONF_FILENAME);
    ...
    /* Open soteria.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Soteria.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Soteria (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Soteria (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Soteria*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "soteria.desktop";
    return GetAutostartDir() / strprintf("soteria-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a soteria.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Soteria\n";
        else
            optionFile << strprintf("Name=Soteria (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the soteria app
...
bool GetStartOnSystemStartup()
{
    CFURLRef soteriaAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (soteriaAppUrl == nullptr) {
        return false;
    }

    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, soteriaAppUrl);

    CFRelease(soteriaAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add soteria app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, soteriaAppUrl, nullptr, nullptr);
