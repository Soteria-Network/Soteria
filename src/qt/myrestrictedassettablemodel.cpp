...
void MyRestrictedAssetsTableModel::updateDisplayUnit(int /*newDisplayUnit*/)
{
    // We don't actually need to remember the "newDisplayUnit" value
    // because we can always ask the OptionsModel when formatting.
    // All we really need here is to notify any views that the headers
    // (the column titles) may have changed.
    headerDataChanged(Qt::Horizontal, 0, columnCount(QModelIndex())-1);
}
