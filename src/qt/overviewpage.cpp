...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(SoteriaUnits::SOTER),
                      ...
                      const QModelIndex &index ) const
    {
        ...
        icon = platformStyle->SingleColorIcon(icon, COLOR_SOTERIA_18A7B7);
...
public:
                      ...
                      const QModelIndex &index) const
    {
        ...
        gradient.setColorAt(0, COLOR_SOTERIA_19827B);
        gradient.setColorAt(1, COLOR_SOTERIA_18A7B7);
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    viewANS = new QAction(tr("View SNS info"), this);
    ...
    viewANS->setObjectName("View SNS");
    ...
    QObject::connect(networkManager, &QNetworkAccessManager::finished,
        this, [=](QNetworkReply* reply) {
            ...
            if (reply->error()) {
                ...
                ui->labelTotal->setText(SoteriaUnits::formatWithUnit(unit, currentBalance + currentUnconfirmedBalance + currentImmatureBalance, false, SoteriaUnits::separatorAlways));
                return;
            }
            ...
            // Get curreny value
             double coinValue = SoteriaUnits::format(0, currentBalance + currentUnconfirmedBalance + currentImmatureBalance, false, SoteriaUnits::separatorAlways).simplified().remove(' ').toDouble() * num;

            // Set total with curreny value
            ui->labelTotal->setText(SoteriaUnits::formatWithUnit(unit, currentBalance + currentUnconfirmedBalance + currentImmatureBalance, false, SoteriaUnits::separatorAlways) + " (" + QString().setNum(coinValue, 'f', 2) + " " + currency.toUpper() + ")");
...
void OverviewPage::handleAssetRightClicked(const QModelIndex &index)
{
    if(assetFilter) {
        ...
        if (action) {
            ...
            else if (action->objectName() == "View SNS") {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    getPriceInfo();
    ui->labelBalance->setText(SoteriaUnits::formatWithUnit(unit, balance, false, SoteriaUnits::separatorAlways));
    ui->labelUnconfirmed->setText(SoteriaUnits::formatWithUnit(unit, unconfirmedBalance, false, SoteriaUnits::separatorAlways));
    ui->labelImmature->setText(SoteriaUnits::formatWithUnit(unit, immatureBalance, false, SoteriaUnits::separatorAlways));
    ui->labelWatchAvailable->setText(SoteriaUnits::formatWithUnit(unit, watchOnlyBalance, false, SoteriaUnits::separatorAlways));
    ui->labelWatchPending->setText(SoteriaUnits::formatWithUnit(unit, watchUnconfBalance, false, SoteriaUnits::separatorAlways));
    ui->labelWatchImmature->setText(SoteriaUnits::formatWithUnit(unit, watchImmatureBalance, false, SoteriaUnits::separatorAlways));
    ui->labelWatchTotal->setText(SoteriaUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, SoteriaUnits::separatorAlways));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    // Reset indents when NOT in watch-only
    ui->labelBalance->setIndent(0);
    ui->labelUnconfirmed->setIndent(0);
    ui->labelImmature->setIndent(0);
    ui->labelTotal->setIndent(0);
    }
    else{
   // Apply indents ONLY in watch-only mode
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("SOTER")
    updateDisplayUnit();
}
...
void OverviewPage::openDataForAsset(const QModelIndex &index, bool forceANS)
{
    ...
    // Get SNS ID of the asset clicked
    QString ansid = index.data(AssetTableModel::AssetANSRole).toString();
    ...
    // Show SNS data if attached to asset
    else if (ansid.count() > 0) {
        CSoteriaNameSystem sns(ansid.toStdString());
        QString ansData = "";

        if(sns.type() == CSoteriaNameSystem::ADDR) ansData = "Address: " + QString::fromStdString(sns.addr());
        if(sns.type() == CSoteriaNameSystem::IPv4) ansData = "IPv4: " + QString::fromStdString(sns.ipv4());

        QMessageBox::information(this, "SNS Info", assetname + " links to:\n" + ansData);
    }
}
...
{
    QString url;
    url = "https://api.coingecko.com/api/v3/coins/soteria-network/";
...
/* void OverviewPage::openDataForAsset(const QModelIndex &index)
{
    // Get the ipfs hash of the asset clicked
    QString ipfshash = index.data(AssetTableModel::AssetIPFSHashRole).toString();
