...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, const POW_TYPE powType)
{
    ...
    // Enforce the active PQ block weight limit.
    nBlockMaxWeight = GetMaxBlockWeightForPrev(pindexPrev, chainparams.GetConsensus()) - 4000;

    pblock->nVersion = ComputeBlockVersion(pindexPrev, chainparams.GetConsensus());

    // Refuse to attempt to create a non-soterg block before activation
    if (!IsDualAlgoEnabled(pindexPrev, chainparams.GetConsensus()) && powType != 0)
        throw std::runtime_error("Error: Won't attempt to create a non-soterg block before Dual_Algo activation");

    // If Dual Algo is enabled, encode desired pow type.
    ...
    if (!IsValidDestination(destFoundationReserve)) {
		...
		return nullptr; // Abort block creation, otherwise the block will be rejected by peers, wasting our mining effort.
    }
	...
	LogPrintf("scriptPubKeyFoundationReserve: %s \n", HexStr(scriptPubKeyFoundationReserve));
	LogPrintf("nFoundationReserveFund: %ld \n", coinbaseTx.vout[1].nValue);
...
void static SoteriaMiner(const CChainParams& chainparams, const POW_TYPE powType)
{
    LogPrintf("SoteriaMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("soteria-miner");
...
int GenerateSoterias(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    if (!algoFound)
        LogPrintf("SoteriaMiner -- Invalid pow algorithm requested");
