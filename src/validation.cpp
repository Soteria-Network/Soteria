...
#if defined(NDEBUG)
# error "Soteria cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Soteria Signed Message:\n";
 ...
 bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (const CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SOTER additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::ConsensusParams& consensusParams)
{
   ...
   CAmount nSubsidy = 5 * COIN / 10000; // Default subsidy (overridden by phase rules, default tail)
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo& txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn& txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** SOTER START */ /* Pass assetCache into function */ /** SOTER END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** SOTER START */ /* Pass assetCache into function */ /** SOTER END */
}
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** SOTER START */ /* Pass assetsCache into the SpendCoin function */ /** SOTER END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** SOTER START */ /* Pass assetsCache into ApplyTxInUndo function */ /** SOTER END */
...
{
    // return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_DALGO, versionbitscache) == THRESHOLD_ACTIVE);
    if (pindexPrev != nullptr) {
        return (pindexPrev->nTime > params.lwmaTimestamp);
    } else {
        return false;
    }
}
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << j << ", " << "true" << ", " << assetAmount * -1 << std::endl;
    ...
    /** SOTER START */
	//FoundationReserveAddress Assign 10%
    ...
    if (!IsValidDestination(destFoundationReserve)) {
		LogPrintf("IsValidDestination: Invalid Soter address %s \n", GetFoundationReserveAddress);
    }
	// Parse address
	...
	CAmount nFoundationReserveFundValue		= nSubsidy*nFoundationReserveFund/100;
	/* Remove Log to console
	LogPrintf("==>block.vtx[0]->vout[1].nValue:    %ld \n", block.vtx[0]->vout[1].nValue);
	LogPrintf("==>nFoundationReserveFundValue: %ld \n", nFoundationReserveFundValue);
	LogPrintf("==>block.vtx[0]->vout[1].scriptPubKey: %s \n", block.vtx[0]->vout[1].scriptPubKey[3]);
	LogPrintf("==>GetFoundationReserveAddress:   %s \n", GetFoundationReserveAddress);
	LogPrintf("==>scriptPubKeyFoundationReserve    Actual: %s \n", HexStr(block.vtx[0]->vout[1].scriptPubKey));
	LogPrintf("==>scriptPubKeyFoundationReserve Should Be: %s \n", HexStr(scriptPubKeyFoundationReserve));
	*/
	//Check 10% Amount
	if(block.vtx[0]->vout[1].nValue != nFoundationReserveFundValue )		{
		return state.DoS(100,
                         error("ConnectBlock(): coinbase Foundation Reserve Fund Is Invalid. Actual: %ld Should be:%ld ",block.vtx[0]->vout[1].nValue, nFoundationReserveFundValue),
                         REJECT_INVALID, "bad-cb-foundation-reserve-fund");
	}
	//Check 10% Address
	if( HexStr(block.vtx[0]->vout[1].scriptPubKey) != HexStr(scriptPubKeyFoundationReserve) )		{
		return state.DoS(100,
                         error("ConnectBlock(): coinbase Foundation Reserve Address Is Invalid. Actual: %s Should Be: %s \n",HexStr(block.vtx[0]->vout[1].scriptPubKey), HexStr(scriptPubKeyFoundationReserve)),
                         REJECT_INVALID, "bad-cb-foundation-reserve-address");
	}
	/** SOTER END */
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState& state, FlushStateMode mode, int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** SOTER START */ /** SOTER END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush SOTER: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (IsDGWActive(pindexPrev->nHeight + 1)) {
        int64_t max_future_block_time = IsDualAlgoEnabled(pindexPrev, consensusParams) ? MAX_FUTURE_BLOCK_TIME_DALGO : MAX_FUTURE_BLOCK_TIME_DGW;
        if (block.GetBlockTime() > nAdjustedTime + max_future_block_time)
            return state.Invalid(false, REJECT_INVALID, "time-too-new", "block timestamp too far in the future");
    } else {
        int64_t max_future_block_time = IsDualAlgoEnabled(pindexPrev, consensusParams) ? MAX_FUTURE_BLOCK_TIME_DALGO : MAX_FUTURE_BLOCK_TIME;
        if (block.GetBlockTime() > nAdjustedTime + max_future_block_time)
            return state.Invalid(false, REJECT_INVALID, "time-too-new", "block timestamp too far in the future");
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::ConsensusParams& consensusParams, const CBlockIndex* pindexPrev, CAssetsCache* assetCache)
{
    ...
    // PQ-aware block weight check (RIP-25).
    int64_t blockWeight = IsPQWitnessDiscountActive(pindexPrev, consensusParams)
                        ? GetBlockWeightRIP25(block)
                        : GetBlockWeight(block);
    unsigned int maxBlockWeight = GetMaxBlockWeightForPrev(pindexPrev, consensusParams);
    ...
    int nLockTimeFlags = 0;
    // we don't want apply MTP at genesis.
    if (consensusParams.nCSVEnabled && nHeight > 0) {
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // Get prev block index. Look up the parent block.
      ...
      // Skip headers validation 
      // Compute height of the new block
     // int nHeight = pindexPrev->nHeight + 1;
     // if (nHeight <= SKIP_BLOCKHEADER) {
     // we’re still far enough behind the tip that we skip all of the
     // expensive header/PoW/contextual checks
     // return true; }
        
        // normal header checks kick in:
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevblk-invalid");
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** SOTER START */ /*Add asset to function */ /** SOTER END*/
...
bool AreEnforcedValuesDeployed()
{
    ...
    // SOTER: Always return true for enforced values.
    return true;
}
...
bool AreCoinbaseCheckAssetsDeployed()
{
    ...
    // SOTER: Always return true for coinbase asset checks.
    return true;
}
...
bool AreAssetsDeployed()
{
    ...
    if ((chainActive.Tip() != nullptr) && chainActive.Tip()->nTime > Params().GetConsensus().vUpgrades[Consensus::SOTERIA_ASSETS].nTimestamp)
        fAssetsIsActive = true;
...
{
    if (fSmartContractsIsActive)
        return true;

    if ((chainActive.Tip() != nullptr) && chainActive.Tip()->nTime > Params().GetConsensus().vUpgrades[Consensus::SOTERIA_SMART_CONTRACTS].nTimestamp)
        fSmartContractsIsActive = true;

    return fSmartContractsIsActive;
}
...
{
    if (fSoteriaNameSystemIsActive)
        return true;

    if ((chainActive.Tip() != nullptr) && chainActive.Tip()->nTime > Params().GetConsensus().vUpgrades[Consensus::SOTERIA_NAME_SYSTEM].nTimestamp)
        fSoteriaNameSystemIsActive = true;

    return fSoteriaNameSystemIsActive;
}
