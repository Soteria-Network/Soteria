...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/SoteriaParams"
else
    PARAMS_DIR="$HOME/.soteria-params"
fi
...
DOWNLOAD_URL="https://github.com/soteria-network/soteria/releases/1.0.0"
...
# fetch methods can be disabled with SOTER_DISABLE_SOMETHING=1
SOTER_DISABLE_WGET="${SOTER_DISABLE_WGET:-}"
#SOTER_DISABLE_IPFS="${SOTER_DISABLE_IPFS:-}"
SOTER_DISABLE_CURL="${SOTER_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SOTER_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SOTER_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SOTER_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Soteria parameters!
...
main() {
    ...
    cat <<EOF
SOTER - fetch-params.sh

This script will fetch the Soteria parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Soteria parameters. Note that it is
