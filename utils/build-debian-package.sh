...
PACKAGE_VERSION=$($SRC_PATH/src/soteriad --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/soteriad $DEB_BIN
cp $SRC_PATH/src/soteria-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/soteria-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/soteriad.1 $DEB_MAN
cp $SRC_DOC/man/soteria-cli.1 $DEB_MAN
cp $SRC_DOC/man/soteria-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/soteriad.bash-completion $DEB_CMP/soteriad
cp $SRC_PATH/contrib/soteria-cli.bash-completion $DEB_CMP/soteria-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/soteriad.1
gzip --best -n $DEB_MAN/soteria-cli.1
gzip --best -n $DEB_MAN/soteria-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/soteriad $DEB_BIN/soteria-cli
