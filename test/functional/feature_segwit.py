...
class SegWitTest(SoteriaTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_unspent(self.nodes[0], 10), self.pubkey[n], False, Decimal("9.9")))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_unspent(self.nodes[0], 10), self.pubkey[n], True, Decimal("9.9")))
        ...
        assert_equal(int(self.nodes[0].getmempoolentry(txid3)["wtxid"], 16), tx.calc_soterg(True))
