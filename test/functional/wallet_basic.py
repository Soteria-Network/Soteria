...
class WalletTest(SoteriaTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(wallet_info['immature_balance'], 10)
        ...
        # Send 21 SOTER from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 20 SOTER in block rewards plus fees, but
        # minus the 21 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), 20-21)
        ...
        assert_equal(self.nodes[2].getbalance(), 19.988)
        assert_equal(self.nodes[2].getbalance("from1"), 19.988-21)

        # Send 10 SOTER normal
        ...
        self.sync_all([self.nodes[0:3]])
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal('19.968'), fee_per_byte, count_bytes(self.nodes[2].getrawtransaction(txid)))
        assert_equal(self.nodes[0].getbalance(), Decimal('10'))

        # Send 10 SOTER with subtract fee from amount
        ...
        # Sendmany 10 SOTER with subtract fee from amount
        ...
        outputs = {self.nodes[1].getnewaddress(): 9.998, self.nodes[0].getnewaddress(): 1111.11}
