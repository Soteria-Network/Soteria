...
`test/functional/test_framework/test_framework.py:SoteriaTestFramework.main`.
...
SKIPPED_TESTS = [
    ...
    'feature_cltv.py',              #TODO - fix mininode rehash methods to use X12R
    'feature_dersig.py',            #TODO - fix mininode rehash methods to use X12R
    'feature_nulldummy.py',         #TODO - fix mininode rehash methods to use X12R
    'feature_pruning.py',
    'feature_rbf.py',
    'feature_segwit.py',            #TODO - fix mininode rehash methods to use X12R
    'mempool_packages.py',
    'mining_basic.py',              #TODO - fix mininode rehash methods to use X12R
    'p2p_compactblocks.py',         #TODO - refactor to assume segwit is always active
    'p2p_fingerprint.py',           #TODO - fix mininode rehash methods to use X12R
    'p2p_segwit.py',                #TODO - refactor to assume segwit is always active
    'p2p_sendheaders.py',           #TODO - fix mininode rehash methods to use X12R
...
def main():
    ...
    tmpdir = "%s/soteria_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and soteriad
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_soteriad = config["components"].getboolean("ENABLE_SOTERIAD")
    if not (enable_wallet and enable_cli and enable_soteriad):
        print("No functional tests to run. Wallet, utils, and soteriad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if soteriad is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "soteriad"]) is not None:
            print("%sWARNING!%s There is already a soteriad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SOTERIAD" not in os.environ:
        os.environ["SOTERIAD"] = build_dir + '/src/soteriad' + exeext
        os.environ["SOTERIACLI"] = build_dir + '/src/soteria-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `soteria-cli help` (`rpc_interface.txt`).
