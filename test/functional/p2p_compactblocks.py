...
# TestNode: A peer we use to send messages to soteriad, and store responses.
class TestNode(NodeConnCB):
    ...
    def on_cmpctblock(self, conn, message):
        ...
        self.last_message["cmpctblock"].header_and_shortids.header.calc_soterg()
...
class CompactBlocksTest(SoteriaTestFramework):
    ...
    def make_utxos(self):
        ...
        self.utxos.extend([[tx.soterg, i, out_value] for i in range(10)])
        return
    ...
    # This test actually causes soteriad to (reasonably!) disconnect us, so do this last.
    ...
    def check_compactblock_construction_from_block(version, header_and_shortids, block_hash, block):
        ...
        header_and_shortids.header.calc_soterg()
        ...
        for entry in header_and_shortids.prefilled_txn:
            entry.tx.calc_soterg()
            # This checks the non-witness parts of the tx agree
            assert_equal(entry.tx.sha256, block.vtx[entry.index].sha256)

            # And this checks the witness
            wtxid = entry.tx.calc_soterg(True)
            if version == 2:
                assert_equal(wtxid, block.vtx[entry.index].calc_soterg(True))
            else:
        ...
        while index < len(block.vtx):
            if (len(header_and_shortids.prefilled_txn) > 0 and
                ...
                if version == 2:
                    tx_hash = block.vtx[index].calc_soterg(True)
                shortid = calculate_shortid(k0, k1, tx_hash)
                assert_equal(shortid, header_and_shortids.shortids[0])
                header_and_shortids.shortids.pop(0)
            index += 1

    # Test that soteriad requests compact blocks when we announce new blocks
    ...
    def test_compactblock_requests(self, node, test_node, version, segwit):
        ...
        for announce in ["inv", "header"]:
            ...
            if version == 2:
                coinbase_hash = block.vtx[0].calc_soterg(True)
            comp_block.shortids = [
    ...
    def build_block_with_transactions(self, node, utxo, num_transactions):
        ...
        for _ in range(num_transactions):
            ...
            tx.rehash()
            utxo = [tx.soterg, 0, tx.vout[0].nValue]
            block.vtx.append(tx)
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for soteriad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # soteriad will not send blocktxn responses for blocks whose height is
        ...
        while current_height >= chain_height - MAX_GETBLOCKTXN_DEPTH:
            ...
            [tx.calc_soterg() for tx in block.vtx]
            with mininode_lock:
                ...
                for index in all_indices:
                    ...
                    if version == 1:
                        ...
                        assert_equal(tx.calc_soterg(True), block.vtx[index].calc_soterg(True))
        ...
        test_node.send_and_ping(msg)
        with mininode_lock:
            test_node.last_message["block"].block.calc_soterg()
    ...
    def test_end_to_end_block_relay(self, node, listeners):
        ...
        with mininode_lock:
            for l in listeners:
                ...
                l.last_message["cmpctblock"].header_and_shortids.header.calc_soterg()
