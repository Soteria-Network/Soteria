...
class UTXO:
    ...
    def __init__(self, soterg, n, n_value):
        self.soterg = soterg
        self.n = n
        self.nValue = n_value
...
class TestNode(NodeConnCB):
    ...
    def announce_tx_and_wait_for_getdata(self, tx, timeout=60):
        ...
        self.send_message(MsgInv(inv=[CInv(1, tx.soterg)]))
    ...
    def announce_block_and_wait_for_getdata(self, block, use_header):
        ...
        if use_header:
            ...
            self.send_message(MsgInv(inv=[CInv(2, block.soterg)]))
...
class SegWitTest(SoteriaTestFramework):
    ...
    def test_non_witness_transaction(self):
        ...
        self.utxo.append(UTXO(tx.soterg, 0, 49 * 100000000))
    ...
    def test_unnecessary_witness_before_segwit_activation(self):
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(self.utxo[0].soterg, self.utxo[0].n), b""))
        ...
        tx.rehash()
        assert (tx.soterg != tx.calc_soterg(with_witness=True))
        ...
        # Right now, soteriad delays sending reject messages for blocks
        ...
        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.soterg, 0), b""))
        ...
        tx3 = CTransaction()
        tx3.vin.append(CTxIn(COutPoint(tx2.soterg, 0), CScript([p2sh_program])))
        ...
        tx4 = CTransaction()
        tx4.vin.append(CTxIn(COutPoint(tx3.soterg, 0), CScript([p2sh_program])))
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(tx4.soterg, 0, tx4.vout[0].nValue))
    ...
    def test_witness_commitments(self):
        ...
        tx3 = CTransaction()
        tx3.vin.append(CTxIn(COutPoint(tx2.soterg, 0), b""))
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(tx3.soterg, 0, tx3.vout[0].nValue))
    ...
    def test_witness_block_size(self):
        ...
        prevout = COutPoint(self.utxo[0].soterg, self.utxo[0].n)
        ...
        for i in range(NUM_OUTPUTS):
            child_tx.vin.append(CTxIn(COutPoint(parent_tx.soterg, i), b""))
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(block.vtx[-1].soterg, 0, block.vtx[-1].vout[0].nValue))
    ...
    def test_submit_block(self):
        ...
        # Now redo commitment with the standard nonce, but let soteriad fill it in.
    ...
    def test_extra_witness_data(self):
        ...
        # witness
        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.soterg, 0), b""))  # witness output
        tx2.vin.append(CTxIn(COutPoint(tx.soterg, 1), b""))  # non-witness
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(tx2.soterg, 0, tx2.vout[0].nValue))
    ...
    def test_max_witness_program_length(self):
        ...
        tx.rehash()
        tx2.vin[0].prevout.hash = tx.soterg
    ...
    def test_witness_input_length(self):
        ...
        for i in range(10):
            tx2.vin.append(CTxIn(COutPoint(tx.soterg, i), b""))
    ...
    def test_tx_relay_after_segwit_activation(self):
        ...
        self.old_node.wait_for_inv([CInv(1, tx2.soterg)])  # wait until tx2 was inv'ed
        self.test_node.test_transaction_acceptance(tx3, with_witness=True, accepted=True)
        self.old_node.wait_for_inv([CInv(1, tx3.soterg)])

        # Test that getrawtransaction returns correct witness information
        # hash, size, vsize
        raw_tx = self.nodes[0].getrawtransaction(tx3.hash, 1)
        assert_equal(int(raw_tx["hash"], 16), tx3.calc_soterg(True))
    ...
    def test_block_relay(self, segwit_activated):
        ...
        if not segwit_activated:
            ...
            non_wit_block = self.test_node.request_block(block.soterg, 2)
            wit_block = self.test_node.request_block(block.soterg, 2 | MSG_WITNESS_FLAG)
            ...
            assert (block4.soterg not in self.old_node.getdataset)
    ...
    def test_standardness_v0(self, segwit_activated):
        ...
        p2sh_tx.vin = [CTxIn(COutPoint(self.utxo[0].soterg, self.utxo[0].n), b"")]
        ...
        tx = CTransaction()
        tx.vin = [CTxIn(COutPoint(p2sh_tx.soterg, 0), CScript([witness_program]))]
        ...
        if segwit_activated:
            ...
            tx2.vin = [CTxIn(COutPoint(tx.soterg, 1), b"")]
            tx2.vout = [CTxOut(7000, scriptPubKey)]
            tx2.wit.vtxinwit.append(CTxInWitness())
            tx2.wit.vtxinwit[0].scriptWitness.stack = [witness_program]
        else:
            # if tx wasn't accepted, we just re-spend the p2sh output we started with.
            tx2.vin = [CTxIn(COutPoint(p2sh_tx.soterg, 0), CScript([witness_program]))]
        ...
        if segwit_activated:
            ...
            tx3.vin = [CTxIn(COutPoint(tx.soterg, 0), b"")]
            ...
            tx3.vin = [CTxIn(COutPoint(p2sh_tx.soterg, 0), CScript([witness_program]))]
    ...
    def test_segwit_versions(self):
        ...
        if len(self.utxo) < NUM_TESTS:
            ...
            self.utxo.pop(0)
            for i in range(NUM_TESTS):
                self.utxo.append(UTXO(tx.soterg, i, split_value))
        ...
        for version in list(range(OP_1, OP_16 + 1)) + [OP_0]:
            ...
            tx.vin = [CTxIn(COutPoint(self.utxo[0].soterg, self.utxo[0].n), b"")]
            tx.vout = [CTxOut(self.utxo[0].nValue - 1000, scriptPubKey)]
            tx.rehash()
            self.std_node.test_transaction_acceptance(tx, with_witness=True, accepted=False)
            self.test_node.test_transaction_acceptance(tx, with_witness=True, accepted=True)
            self.utxo.pop(0)
            temp_utxo.append(UTXO(tx.soterg, 0, tx.vout[0].nValue))
        ...
        tx2 = CTransaction()
        tx2.vin = [CTxIn(COutPoint(tx.soterg, 0), b"")]
        ...
        temp_utxo.append(UTXO(tx2.soterg, 0, tx2.vout[0].nValue))

        # Spend everything in temp_utxo back to an OP_TRUE output.
        tx3 = CTransaction()
        total_value = 0
        for i in temp_utxo:
            tx3.vin.append(CTxIn(COutPoint(i.soterg, i.n), b""))
            tx3.wit.vtxinwit.append(CTxInWitness())
    ...
    def test_premature_coinbase_witness_spend(self):
        ...
        spend_tx.vin = [CTxIn(COutPoint(block.vtx[0].soterg, 0), b"")]
    ...
    def test_signature_version_1(self):
        ...
        # Test each hashtype
        prev_utxo = UTXO(tx.soterg, 0, tx.vout[0].nValue)
        for sigflag in [0, SIGHASH_ANYONECANPAY]:
            for hashtype in [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE]:
                hashtype |= sigflag
                block = self.build_next_block()
                tx = CTransaction()
                tx.vin.append(CTxIn(COutPoint(prev_utxo.soterg, prev_utxo.n), b""))
        ...
        for i in range(NUM_TESTS):
            temp_utxos.append(UTXO(tx.soterg, i, split_value))
        ...
        for i in range(NUM_TESTS):
            ...
            for j in range(num_inputs):
                tx.vin.append(CTxIn(COutPoint(temp_utxos[j].soterg, temp_utxos[j].n), b""))
                tx.wit.vtxinwit.append(CTxInWitness())
            ...
            for idx in range(num_outputs):
                temp_utxos.append(UTXO(tx.soterg, idx, split_value))
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(temp_utxos[0].soterg, temp_utxos[0].n), b""))
        ...
        for i in temp_utxos:
            ...
            tx.vin.append(CTxIn(COutPoint(i.soterg, i.n), b""))
        ...
        for i in range(len(tx.vout)):
            self.utxo.append(UTXO(tx.soterg, i, tx.vout[i].nValue))
    ...
    def test_p2sh_witness(self, segwit_activated):
        ...
        spend_tx = CTransaction()
        spend_tx.vin.append(CTxIn(COutPoint(tx.soterg, 0), scriptSig))
        ...
        # segwit activation.  Note that older soteriad's that are not
        ...
        # Update self.utxo
        self.utxo.pop(0)
        self.utxo.append(UTXO(spend_tx.soterg, 0, spend_tx.vout[0].nValue))

    # Test the behavior of starting up a segwit-aware node after the softfork
    # has activated.  As segwit requires different block data than pre-segwit
    # nodes would have stored, this requires special handling.
    # To enable this test, pass --oldbinary=<path-to-pre-segwit-soteriad> to
    # the test.
    def test_upgrade_after_activation(self, node_id):
        self.log.info("Testing software upgrade after softfork activation")

        assert (node_id != 0)  # node0 is assumed to be a segwit-active soteriad
    ...
    def test_witness_sigops(self):
        ...
        tx2.vout.pop()
        tx2.vin.append(CTxIn(COutPoint(tx.soterg, outputs - 1), b""))
        tx2.wit.vtxinwit.append(CTxInWitness())
    ...
    def test_uncompressed_pubkey(self):
        ...
        scriptPKH = CScript([OP_0, pubkeyhash])
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(utxo.soterg, utxo.n), b""))
        ...
        tx4 = CTransaction()
        tx4.vin.append(CTxIn(COutPoint(tx3.soterg, 0), scriptSig))
        ...
        # transactions.
        tx5 = CTransaction()
        tx5.vin.append(CTxIn(COutPoint(tx4.soterg, 0), b""))
        ...
        self.test_node.test_witness_block(block, accepted=True)
        self.utxo.append(UTXO(tx5.soterg, 0, tx5.vout[0].nValue))
    ...
    def test_non_standard_witness(self):
        ...
        for i in range(len(scripts)):
            ...
            p2wsh_tx.vout.append(CTxOut(outputvalue - 10, CScript([OP_0, hash160(hex_str_to_bytes(""))])))
            p2wsh_tx.wit.vtxinwit.append(CTxInWitness())
            p2wsh_tx.rehash()
            p2wsh_txs.append(p2wsh_tx)
            p2sh_tx = CTransaction()
            p2sh_tx.vin.append(CTxIn(COutPoint(txid, i * 2 + 1), CScript([p2wsh_scripts[i]])))
            p2sh_tx.vout.append(CTxOut(outputvalue - 10, CScript([OP_0, hash160(hex_str_to_bytes(""))])))
