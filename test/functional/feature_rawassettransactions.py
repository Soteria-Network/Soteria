...
def get_first_unspent(self: object, node: object, needed: float = 1.1) -> object:
...
def get_tx_issue_hex(self, node, asset_name, asset_quantity, asset_units=0):
    ...
    outputs = {
        'n1issueAssetXXXXXXXXXXXXXXXXWdnemQ': 5,
        change_address: truncate(float(unspent['amount']) - 5.1),
        to_address: {
            'issue': {
...
class RawAssetTransactionsTest(SoteriaTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 3

    def activate_assets(self):
        self.log.info("Generating SOTER for node[0] and activating assets...")
        n0 = self.nodes[0]
    ...
    def reissue_tampering_test(self):
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 1,
            n0.getnewaddress(): truncate(float(unspent['amount']) - 1.1),
            to_address: {
                'reissue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1000,
        ...
        tx.deserialize(f)
        soterr = '72766e72'  # soterr
        op_drop = '75'
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if soterr in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                reissue_script_hex = script_hex[script_hex.index(soterr) + len(soterr):-len(op_drop)]
                ...
                tampered_script = script_hex[:script_hex.index(soterr)] + soterr + tampered_reissue + op_drop
        ...
        f = BytesIO(hex_str_to_bytes(tx_hex))
        tx.deserialize(f)
        sotert = '72766e74'  # sotert
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: sotert not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_hex_bad = bytes_to_hex_str(tx.serialize())
        tx_signed = n0.signrawtransaction(tx_hex_bad)['hex']
    ...
    def issue_tampering_test(self):
        ...
        tx.deserialize(f)
        sotero = '72766e6f'  # sotero
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: sotero not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        ...
        owner_vout = list(filter(lambda out_script: sotero in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))[0]
        ...
        f = BytesIO(hex_str_to_bytes(tx_issue_hex))
        tx.deserialize(f)
        soterq = '72766e71'  # soterq
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: soterq not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-bad-asset-transaction",
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if sotero in bytes_to_hex_str(out.scriptPubKey):
                owner_out = out
                owner_script_hex = bytes_to_hex_str(owner_out.scriptPubKey)
                asset_script_hex = owner_script_hex[owner_script_hex.index(sotero) + len(sotero):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                owner = CScriptOwner()
                owner.deserialize(f)
                owner.name = b"NOT_MY_ASSET!"
                tampered_owner = bytes_to_hex_str(owner.serialize())
                tampered_script = owner_script_hex[:owner_script_hex.index(sotero)] + sotero + tampered_owner + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        ...
        tx.deserialize(f)
        sotero = '72766e6f'  # sotero
        SOTERO = '52564e4f'  # SOTERO
        # change the owner output script type to be invalid
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if sotero in bytes_to_hex_str(out.scriptPubKey):
                owner_script_hex = bytes_to_hex_str(out.scriptPubKey)
                tampered_script = owner_script_hex.replace(sotero, SOTERO)
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-soter-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def issue_reissue_transfer_test(self):
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 1,
            change_address: truncate(float(unspent['amount']) - 1.1),
            to_address: {
                'reissue': {
                    'asset_name':       'TEST_ASSET',
                    'asset_quantity':   1000,
        ...
        for i in range(1, 3):
            ...
            f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(sotert) + len(sotert):-len(op_drop)]))
            ...
            tampered_script = script_hex[:script_hex.index(sotert)] + sotert + tampered_transfer + op_drop
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if sotert in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(sotert) + len(sotert):-len(op_drop)]))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
    ...
    def bad_ipfs_hash_test(self):
        ...
        outputs = {
            'n1issueAssetXXXXXXXXXXXXXXXXWdnemQ': 5,
            change_address: truncate(float(unspent['amount']) - 5.0001),
            to_address: {
                'issue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1,
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 1,
            change_address: truncate(float(unspent['amount']) - 1.0001),
            to_address: {
                'reissue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1000,
    ...
    def issue_sub_multiple_outputs_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if sotert in bytes_to_hex_str(out.scriptPubKey):
                ...
                asset_script_hex = transfer_script_hex[transfer_script_hex.index(sotert) + len(sotert):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
                transfer.amount = 0
                tampered_transfer = bytes_to_hex_str(transfer.serialize())
                tampered_script = transfer_script_hex[:transfer_script_hex.index(sotert)] + sotert + tampered_transfer + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
    ...
    def transfer_asset_inserting_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if sotert in bytes_to_hex_str(out.scriptPubKey):
                ...
                # Generate a script that has a valid destination address but switch it with soterq and the issue_serialized data
                issue_out.scriptPubKey = hex_str_to_bytes(transfer_script_hex[:transfer_script_hex.index(sotert)] + soterq + issue_serialized + op_drop)
    ...
    def atomic_swaps_test(self):
        ...
        # n1 buys 400 ANDUIN from n2 for 8 SOTER
        price = 8
        amount = 0.8
        fee = 0.01
        ...
        # n2 buys JAINA! from n1 for 20000 SOTER
        price = 20000
        amount = 1
    ...
    def fundrawtransaction_transfer_outs(self):
        ...
        for _ in range(0, 5):
            n0.sendtoaddress(n2_address, soter_amount / 5)
        n0.generate(1)
        self.sync_all()
