...
def check_json_precision():
    """Make sure json library being used does not lose precision converting SOTER values"""
...
soterg_hash_cmd = os.path.dirname(os.path.realpath(__file__)) + "/../../../src/test/test_soteria_hash"


def x16_hash_block(hex_str, algorithm="2"):
    """
    :param hex_str:     Blockhash to convert
    :param algorithm:   Which algorithm ~~ "1" = soterg  "2" = sotergv2
    :return:            Converted hash
    """
    cmd = [soterg_hash_cmd, hex_str, algorithm]
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "soteria.conf")):
        with open(os.path.join(datadir, "soteria.conf"), 'r', encoding='utf8') as f:
            for line in f:
