...
"""
Soteria test framework primitive and message structures.
...
COIN = 100000000  # 1 soter in Corbies
...
class CAddress:
    """
    Objects that map to soteriad objects, which can be serialized/deserialized
    """
...
class CTransaction:
    __slots__ = ("nVersion", "vin", "vout", "wit", "nLockTime", "soterg", "hash")
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in soteriad
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
    ...
    # self.soterg and self.hash -- those are expected to be the txid.
    def calc_soterg(self, with_witness=False):
        if with_witness:
            # Don't cache the result, just return it
            return uint256_from_str(hash256(self.serialize_with_witness()))

        if self.soterg is None:
            self.soterg = uint256_from_str(hash256(self.serialize_without_witness()))
...
class CBlockHeader:
    __slots__ = ("nVersion", "hashPrevBlock", "hashMerkleRoot", "nTime", "nBits", "nNonce", "soterg", "hash")
...
class CBlock(CBlockHeader):
    ...
    def calc_merkle_root(self):
        ...
        for tx in self.vtx:
            tx.calc_soterg()
            hashes.append(ser_uint256(tx.soterg))
    ...
    def calc_witness_merkle_root(self):
        ...
        for tx in self.vtx[1:]:
            ...
            hashes.append(ser_uint256(tx.calc_soterg(True)))
    ...
    def initialize_from_block(self, block, nonce=0, prefill_list=None, use_witness=False):
        ...
        for i in range(len(block.vtx)):
            if i not in prefill_list:
                ...
                if use_witness:
                    tx_hash = block.vtx[i].calc_soterg(with_witness=True)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in soteriad indicates these should be deserialized as blocks
