...
The Soteria Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play soterias" on the test network, if you
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8323 for connections and services them.

- SoteriaMiner : Generates soterias (if wallet is enabled).
...
However, in open source software such as Soteria Core, where everyone uses
...
pay attention to for reviewers of Soteria Core code.

General Soteria Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Soteria Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Soteria so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:soteria/soteria.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `soteria-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `soteria-cli` and the GUI debug console use this table to determine how to
