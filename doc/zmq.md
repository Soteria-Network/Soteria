...
The Soteria Core daemon can be configured to act as a trusted "border
router", implementing the soteria wire protocol and relay, making
...
The ZeroMQ feature in Soteria Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building soteriad:
...
For instance:
    ...
    $ soteriad -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/soteriad.tx.raw
...
These options can also be provided in soteria.conf.
...
From the perspective of soteriad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into soteriad directly. Furthermore, no information is
...
using. Soteriad appends an up-counting sequence number to each
