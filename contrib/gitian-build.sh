...
# Dependencies
ossPatchUrl="https://depends.soteriacore.site/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="hash placeholder"

ossTarUrl="https://depends.soteriacore.site/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="hash placeholder"

macosSdkUrl="https://depends.soteriacore.site/MacOSX11.3.sdk.tar.gz"
macosSdkHash="hash placeholder"

liefUrl="https://depends.soteriacore.site/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
liefHash="hash placeholder"
...
SIGNER=
VERSION=
url=https://github.com/soteria-network/soteria
proc=2
mem=2000
...
Standalone script to perform the gitian build of Soteria Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/soteria-network/soteria
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/soteria-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/soteria-network/gitian.sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/soteria-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean soteria git directory because of old caching
    if [ -d inputs/soteria/ ]; then
        echo "Cleaning Soteria directory..."
        rm -rf inputs/soteria/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit soteria="$COMMIT" --url soteria="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
