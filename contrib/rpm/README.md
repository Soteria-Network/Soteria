...
The RPM spec file provided here is for Soteria-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://soter.network/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Soteria git repository](https://github.com/SoteriaNetwork/Soteria) but are not part of
...
Currently only `Source10` is used, for the example `soteria.conf` file.
...
in the soteria git repository. At present only an SVG version of the Soteria
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the soteria
...
needed when 0.12.1 is released, a proper fix is already in the Soteria git
master branch.
...
idea to build Soteria Core against a static version of that library compiled
...
The `soteria.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `soteria-server` package. At this point, most RPM based
...
The `soteria-server` package should have SELinux support. How to properly do
...
Soteria-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/soteria/). He has been
packaging Soteria for Fedora at least since 2012.
...
4. I renamed the `soteria` package that contains the Qt GUI to `soteria-core` as
that appears to be how the general population refers to it, in contrast to
`soteria-xt` or `soteria-classic`. I wanted to make sure the general population
