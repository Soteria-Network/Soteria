...
From an ubuntu 24.04 / Debian Trixie (required)
```
cd ~/
export PATH_orig=$PATH
DISTNAME=soteria-1.0.0
sudo apt install -y curl g++-aarch64-linux-gnu g++-12-aarch64-linux-gnu gcc-12-aarch64-linux-gnu binutils-aarch64-linux-gnu g++-arm-linux-gnueabihf g++-12-arm-linux-gnueabihf gcc-12-arm-linux-gnueabihf binutils-arm-linux-gnueabihf g++-12-multilib gcc-12-multilib binutils-gold git pkg-config autoconf libtool automake bsdmainutils ca-certificates python
...
sudo find ${DISTNAME}/bin -type f -executable -exec ../soteria/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../soteria/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/soteria-*-win*-unsigned.tar.gz to the signing machine
```


# Signing windows 64 binaries #
From an ubuntu 24.04 / Debian Trixie !important (openssl 1.1.1 required)  
...
#transfer soteria-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer soteria-*-win*-setup.exe back to the build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer soteria-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
# Transfer soteria-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
~/soteria/contrib/macdeploy/extract-osx-sdk.sh
rm -rf 5.hfs MacOSX11.3.sdk Xcode_7.3.1.dmg
# Save MacOSX11.3.sdk.tar.gz somewhere safe for future builds
```
...
git clone https://github.com/soteria-network/soteria
mkdir ~/soteria/depends/SDKs
#transfer MacOSX11.3.sdk.tar.gz to the folder soteria/depends/SDKs
cd soteria/depends/SDKs && tar -xf MacOSX11.3.sdk.tar.gz 
rm MacOSX11.3.sdk.tar.gz 
cd ~/soteria/depends
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/soteria-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer soteria-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
...
```
From ubuntu 24.04 / Debian Trixie (required) build machine:
```
DISTNAME=soteria-1.0.0
cd ~/sign
mkdir OSX/
