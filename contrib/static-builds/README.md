...
From Ubuntu 24.04 / Debian Trixie
...
Save MacOSX11.3.sdk.tar.gz to the ~/ folder of your build machine

Static Build process
---------------------

Run the automated shell script from ~/ on your machine:  
...
```
Transfer soteria-*-win*-unsigned.tar.gz to ~/win64/ from the Debian 12 Bookworm build machine's ~/sign directory  
```
...
```
Transfer soteria-*-win*-setup.exe back to the Ubuntu 24.04 build machine to the folder ~/release (to shasum with the rest of the releases)
...
```
Transfer soteria-*-win*-unsigned.tar.gz to ~/win32/ from the Ubuntu 18 build machine's ~/sign directory  
```
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
...
mkdir OSX
```
Transfer soteria-*-osx-unsigned.tar.gz to ~/desktop/OSX from the Ubuntu 18 build machine's ~/sign directory 
```
cd OSX
...
Copy signature-osx.tar.gz back to the ubuntu 24.04 build machine
