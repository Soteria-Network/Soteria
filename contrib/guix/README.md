# Bootstrappable Soteria Core Builds

This directory contains the files necessary to perform bootstrappable Soteria
Core builds.
...
To build Soteria Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Soteria Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/soteria-detached-sigs> ./contrib/guix-codesign
```
...
repository](https://github.com/soteria-network/guix.sigs).
