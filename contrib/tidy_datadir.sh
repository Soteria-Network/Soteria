...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Soteria database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Soteria datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Soteria datadir (before 1.0.0)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Soteria 1.0.0 datadir."
    ;;
  3)
    echo "Detected Soteria pre-1.0.0 datadir."
    ;;
  4)
    echo "Detected Soteria 1.0.0 datadir."
    ;;
