...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']}]
        # Soteria: Fee is exact, do not use tolerance
        ...
        assert_raises_jsonrpc(-5, "changeAddress must be a valid soteria address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']} ]
        # Soteria: Reduce this output so the fee doesn't leave us with no change
        ...
        #fund a tx with ~20 small inputs, by spending in combination the 22 SOTER we just sent
        inputs = []
        # Soteria: TX size rounding gives us a fee of 4 SOTER
